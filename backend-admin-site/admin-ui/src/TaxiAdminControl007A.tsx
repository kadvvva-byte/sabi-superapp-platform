...
const COPY006Z: Record<AdminLanguage, { title: string; subtitle: string; noFake: string; refresh: string; loadData: string; queues: string; selected: string; execute: string; reports: string; sources: string; technical: string; empty: string; routeMissing: string; reason: string; lastResponse: string; }> = {
  ru: {
    title: "Такси — порядок управления: заявки, жалобы, баланс и учёт",
    subtitle: "Порядок работы как в настоящей админке: загрузить данные, выбрать очередь, изучить заявку или жалобу, проверить фото и документы, указать причину, выполнить серверное действие и дождаться синхронизации.",
...
const SOURCES006Z: SourceEndpoint006Z[] = [
  ...
  { key: "dbWriteGate", title: "Write gate check", path: "/api/taxi/002x/db-write-runtime/write-gate", method: "POST", description: "Real write gate check; no mutation by UI", headers: { "x-sabi-taxi-db-write-gate": "approve-002x-route-patch-only-no-write" }, body: { source: "admin-ui-007a", requestedAction: "write_gate_check", fakeSuccessBlocked: true } },
  { key: "providerBoundary", title: "Provider / Wallet boundary", path: "/api/taxi/003d/provider-wallet-boundary/check", method: "POST", description: "Provider, Wallet, payment and payout boundary", headers: { "x-sabi-taxi-provider-wallet-boundary": "approve-003d-route-patch-only-no-execution" }, body: { source: "admin-ui-007a", requestedAction: "boundary_check", fakeSuccessBlocked: true } },
  { key: "cockpit", title: "Admin cockpit", path: "/api/taxi/003h/admin-readiness-cockpit/check", method: "POST", description: "Taxi Admin cockpit boundary", headers: { "x-sabi-taxi-admin-cockpit-boundary": "approve-003h-route-patch-only-no-execution" }, body: { source: "admin-ui-007a", requestedAction: "cockpit_check", fakeSuccessBlocked: true } },
];
...
async function call006Z(config: AdminApiConfig, endpoint: SourceEndpoint006Z): Promise<SourceResult006Z> {
  ...
  try {
    const response = await fetch(`${baseUrl006Z(config)}${endpoint.path}`, {
      ...
      body: endpoint.method === "GET" ? undefined : JSON.stringify(endpoint.body || { source: "admin-ui-007a", fakeSuccessBlocked: true }),
    });
...
function makeQueues006Z(dbPayload: unknown): WorkQueue006Z[] {
  const specs = [
    ...
    { key: "rewards", title: "Gifts / rewards boundary", tone: "locked" as Tone006Z, names: ["gifts", "giftRewards", "rewards", "rewardFreezeQueue", "tips", "driverRewards"], description: "Gift/tip/reward review, freeze/release boundary, no Wallet/payout fake", idKeys: ["rewardId", "giftId", "settlementId", "driverId", "id"], statusKeys: ["status", "rewardStatus", "giftStatus", "freezeStatus"], actions: ["reward-freeze-review", "reward-release-review"], columns: ["rewardId", "giftId", "driverId", "amount", "status", "createdAt"] },
  ];
...
function OperationsOrder007A(props: { activeQueue: string; selectedRecord: AnyRecord006Z | null; actionTitle: string; missing: string[]; busy: string; results: Record<string, SourceResult006Z> }) {
  const steps = [
    { n: "1", title: "Загрузить реальные данные", text: "Готовность, маршруты, диагностика и строки такси только для чтения должны прийти от сервера.", done: Boolean(props.results.readiness || props.results.dbRead), tone: props.results.dbRead?.ok ? "ready" : props.results.readiness ? "warn" : "info" as Tone006Z },
    { n: "2", title: "Выбрать очередь", text: "Заявки, жалобы, баланс, авто, заказы, поездки, тарифы или награды.", done: Boolean(props.activeQueue), tone: "ready" as Tone006Z },
    { n: "3", title: "Изучить запись", text: "Открыть карточку: водитель, авто, документы, фото, жалобы, баланс и риски.", done: Boolean(props.selectedRecord), tone: props.selectedRecord ? "ready" : "locked" as Tone006Z },
    { n: "4", title: "Проверить пункты", text: "Проверка личности, авто, фото и документы, безопасность, риск мошенничества и граница баланса.", done: props.missing.length === 0 && Boolean(props.selectedRecord), tone: props.missing.length === 0 && props.selectedRecord ? "ready" : "warn" as Tone006Z },
    { n: "5", title: "Выполнить действие", text: props.actionTitle || "Серверное действие выполняется только после причины администратора.", done: false, tone: props.busy ? "info" : "locked" as Tone006Z },
    { n: "6", title: "Синхронизация", text: "После успешного ответа сервера интерфейс заново загружает данные только для чтения. Локального одобрения нет.", done: false, tone: "safe" as Tone006Z },
  ];
  return <section className="card taxi007AOrder" data-admin-ui-007a-order-flow="true">
    <header className="taxi006ZSectionHeader"><div><h3>Порядок работы админки такси</h3><p>Идём строго слева направо: данные → очередь → изучение → проверка → серверное действие → повторная синхронизация.</p></div><Badge006Z tone="safe">no fake workflow</Badge006Z></header>
    <div className="taxi007AStepper">{steps.map((step) => <article key={step.n} className={step.done ? "done" : ""}><b>{step.n}</b><div><strong>{step.title}</strong><small>{step.text}</small></div><Badge006Z tone={step.tone}>{step.done ? "done" : step.tone}</Badge006Z></article>)}</div>
  </section>;
}

function SyncToolbar007A(props: { busy: string; refreshAll: (includeDbRead?: boolean) => Promise<void>; runSource: (endpoint: SourceEndpoint006Z) => Promise<void>; copy: (typeof COPY006Z)[AdminLanguage]; }) {
  const dbRead = SOURCES006Z.find((item) => item.key === "dbRead")!;
  const primary = SOURCES006Z.filter((item) => ["readiness", "routes", "diagnostics"].includes(item.key));
  const gates = SOURCES006Z.filter((item) => ["dbReadPlan", "dbWritePlan", "dbWriteGate", "providerBoundary", "cockpit"].includes(item.key));
  return <section className="card taxi007ASync" data-admin-ui-007a-sync-panel="true">
    <header className="taxi006ZSectionHeader"><div><h3>1. Синхронизация с основой такси</h3><p>Сначала загружаем состояние сервера. Если сервер заблокирован — интерфейс показывает причину блокировки, а не рисует данные.</p></div><Badge006Z tone={props.busy ? "info" : "ready"}>{props.busy || "ready"}</Badge006Z></header>
    <div className="taxi007APrimaryActions">
      <button type="button" onClick={() => void props.refreshAll(true)} disabled={Boolean(props.busy)}>{props.busy === "refresh-all" ? "Синхронизация..." : "Загрузить всё: данные и ворота проверки"}</button>
      <button type="button" onClick={() => void props.runSource(dbRead)} disabled={Boolean(props.busy)}>Получить живые строки такси</button>
    </div>
    <div className="taxi007AMiniActions"><strong>Быстрая проверка:</strong>{primary.map((endpoint) => <button key={endpoint.key} type="button" onClick={() => void props.runSource(endpoint)} disabled={Boolean(props.busy)}>{endpoint.title}</button>)}</div>
    <details className="taxi007AGates"><summary>Ворота и границы проверки: запись в базу, провайдер, кошелёк, панель управления</summary><div>{gates.map((endpoint) => <button key={endpoint.key} type="button" onClick={() => void props.runSource(endpoint)} disabled={Boolean(props.busy)}>{endpoint.title}<small>{endpoint.method} {endpoint.path}</small></button>)}</div></details>
  </section>;
}

export function TaxiAdminControl007APanel(props: { language: AdminLanguage; config: AdminApiConfig; setNotice?: (message: string) => void }) {
  ...
  return <section className="panel taxi006Z" data-admin-ui-007a-taxi-ordered-control="true" data-admin-ui-006v-route-screen="taxi">
    <div className="taxi006ZHero">
      <div>
        <span>ADMIN-UI 007A · Taxi ordered operations · real backend workflow only</span>
        <h2>{copy.title}</h2>
        <p>{copy.subtitle}</p>
      </div>
      <aside>
    ...
    <OperationsOrder007A activeQueue={activeQueue} selectedRecord={selectedRecord} actionTitle={action.title} missing={missing} busy={busy} results={results} />

    <SyncToolbar007A busy={busy} refreshAll={refreshAll} runSource={runSource} copy={copy} />
    ...
    <section className="taxi007AWorkspace" data-admin-ui-007a-ordered-workspace="true">
      <div className="taxi007AQueueColumn">
    <section className="card taxi006ZQueues" data-admin-ui-006z-management-queues="true">
      ...
      <div className="taxi007ADossierColumn">
        <Dossier006Z record={selectedRecord} queueKey={activeQueue} form={form} setForm={setForm} />
      </div>
    </section>

    <div className="taxi006ZMainGrid taxi007ADecisionGrid" data-admin-ui-007a-decision-grid="true">
