...
type JsonRecord007G = Record<string, unknown>;
type QueueKey007G = "applications" | "drivers" | "vehicles" | "orders" | "trips" | "complaints" | "balance" | "tariffs" | "gifts" | "reports";
type WorkTab007G = "applications" | "complaints" | "accounting" | "fleet" | "operations" | "reports";
type Method007G = "GET" | "POST" | "PATCH" | "PUT" | "DELETE";
type Tone007G = "ready" | "warn" | "locked" | "danger" | "info";

type Source007G = { key: string; title: string; path: string; method: Method007G; body?: JsonRecord007G };
type BackendResult007G = { key: string; title: string; path: string; method: Method007G; ok: boolean; status: number; message: string; payload: unknown; at: string; ms: number };
type ActionJournal007G = { id: string; title: string; queue: QueueKey007G; recordId: string; ok: boolean; status: number; message: string; at: string; route: string; synced: boolean };
type Route007G = { key?: string; operationKey?: string; area?: string; method?: string; path?: string; safeDisabledUntilNextStage?: boolean; requiresAdmin?: boolean; requiresIdempotencyForWrite?: boolean; requiresProviderReadiness?: boolean; requiresWalletBoundary?: boolean };
type Queue007G = { key: QueueKey007G; title: string; subtitle: string; rows: JsonRecord007G[]; requiredReview: string[]; tone: Tone007G };
type Action007G = { key: string; queue: QueueKey007G; title: string; description: string; routeHints: string[]; idKeys: string[]; needsReason: boolean; needsOwnerApproval?: boolean; tone: Tone007G; payloadKind: string };
type Props007G = { language: AdminLanguage; config: AdminApiConfig; setNotice: (message: string) => void };

const MARKER007G = "ADMIN-UI-TAXI-007G-COMPLAINT-REVIEW-CONTROL";
const DASH = "—";

const COPY007G: Record<AdminLanguage, Record<string, string>> = {
  ru: {
...
function tx007G(language: AdminLanguage, key: string): string { return COPY007G[language]?.[key] ?? COPY007G.ru[key] ?? key; }
function isRecord007G(value: unknown): value is JsonRecord007G { return typeof value === "object" && value !== null && !Array.isArray(value); }
function text007G(value: unknown): string { if (value === null || value === undefined || value === "") return DASH; if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") return String(value); if (Array.isArray(value)) return `${value.length} items`; if (typeof value === "object") return "object"; return String(value); }

const SOURCES007G: Source007G[] = [
  ...
  { key: "writeGate", title: "Write gate check", method: "POST", path: "/api/taxi/002x/db-write-runtime/write-gate", body: { source: MARKER007G, action: "check_write_gate", fakeSuccessBlocked: true } },
  { key: "providerWallet", title: "Provider / Wallet boundary", method: "POST", path: "/api/taxi/003d/provider-wallet-boundary/check", body: { source: MARKER007G, action: "check_provider_wallet_boundary", fakeSuccessBlocked: true } },
  { key: "cockpit", title: "Admin cockpit", method: "POST", path: "/api/taxi/003h/admin-readiness-cockpit/check", body: { source: MARKER007G, action: "check_admin_cockpit", fakeSuccessBlocked: true } },
];

const QUEUE_META007G: Record<QueueKey007G, Omit<Queue007G, "key" | "rows">> = {
...
const WORK_TABS007G: Array<{ key: WorkTab007G; title: string; queues: QueueKey007G[]; description: string }> = [
...
function flattenRecords007G(value: unknown, hint: string): JsonRecord007G[] {
  const out: JsonRecord007G[] = [];
...
function collectRoutes007G(value: unknown): Route007G[] {
  const routes: Route007G[] = [];
  const seen = new Set<unknown>();
  const walk = (node: unknown, depth: number) => {
    ...
    if (path && (method || operationKey)) routes.push(node as Route007G);
...
function queueRows007G(results: BackendResult007G[]): Record<QueueKey007G, JsonRecord007G[]> {
  const payloads = results.map((item) => item.payload);
  const all = (hint: string) => payloads.flatMap((payload) => flattenRecords007G(payload, hint));
  return {
...
function firstId007G(row: JsonRecord007G | null, keys: string[]): string {
  if (!row) return "";
...
function pickFields007G(row: JsonRecord007G | null, patterns: RegExp[], limit = 8): Array<[string, unknown]> {
  if (!row) return [];
  return Object.entries(row).filter(([key]) => patterns.some((pattern) => pattern.test(key))).slice(0, limit);
}

function photoFields007G(row: JsonRecord007G | null): Array<[string, string]> {
  if (!row) return [];
...
function findRoute007G(routes: Route007G[], action: Action007G): Route007G | null {
...
function resultMessage007G(payload: unknown, status: number): string {
  if (isRecord007G(payload)) {
...
function FieldGroup007G({ title, rows, empty }: { title: string; rows: Array<[string, unknown]>; empty: string }) {
  return <article className="taxi007g-fieldGroup"><h3>{title}</h3>{rows.length ? <div>{rows.map(([key, value]) => <p key={key}><span>{key}</span><b>{text007G(value)}</b></p>)}</div> : <em>{empty}</em>}</article>;
}

export function TaxiAdminControl007GPanel({ language, config, setNotice }: Props007G) {
  const [busy, setBusy] = useState(false);
  const [results, setResults] = useState<BackendResult007G[]>([]);
  const [last, setLast] = useState<BackendResult007G | null>(null);
  const [activeTab, setActiveTab] = useState<WorkTab007G>("applications");
  const [activeQueue, setActiveQueue] = useState<QueueKey007G>("applications");
  const [selected, setSelected] = useState<JsonRecord007G | null>(null);
  ...
  const [actionJournal, setActionJournal] = useState<ActionJournal007G[]>([]);
  const [queueSearch, setQueueSearch] = useState("");
  const [sourceOpen, setSourceOpen] = useState(false);

  const callSource = useCallback(async (source: Source007G): Promise<BackendResult007G> => {
    ...
    try {
      response = await fetch(`${config.baseUrl}${source.path}`, {
        ...
        body: source.method === "GET" ? undefined : JSON.stringify(source.body ?? { source: MARKER007G, fakeSuccessBlocked: true }),
      });
    ...
    const message = response?.ok ? "ok" : resultMessage007G(payload, status);
  ...
  const refreshAll = useCallback(async () => {
    setBusy(true);
    try {
      const loaded = await Promise.all(SOURCES007G.map((source) => callSource(source)));
      setResults(loaded);
  ...
  const routes = useMemo(() => results.flatMap((item) => item.key === "routes" ? collectRoutes007G(item.payload) : []), [results]);
  const rowsByQueue = useMemo(() => queueRows007G(results), [results]);
  const queues = useMemo(() => (Object.keys(QUEUE_META007G) as QueueKey007G[]).map((key) => ({ key, ...QUEUE_META007G[key], rows: rowsByQueue[key] ?? [] })), [rowsByQueue]);
  const currentTab = WORK_TABS007G.find((tab) => tab.key === activeTab) ?? WORK_TABS007G[0];
  ...
  const actions = ACTIONS007G.filter((action) => action.queue === active.key);
  const action = actions.find((item) => item.key === actionKey) ?? actions[0] ?? ACTIONS007G[0];
  const route = useMemo(() => findRoute007G(routes, action), [routes, action]);
  const selectedId = manualId.trim() || firstId007G(selected, action.idKeys);
  ...
  useEffect(() => {
    ...
    const firstAction = ACTIONS007G.find((item) => item.queue === firstQueue) ?? ACTIONS007G[0];
  ...
  useEffect(() => {
    ...
    setChecked({});
    setManualId("");
    const firstAction = ACTIONS007G.find((item) => item.queue === active.key) ?? ACTIONS007G[0];
    setActionKey(firstAction.key);
  }, [active.key, active.rows]);

  const runAction = async () => {
    if (!route?.path || !route.method) return;
    const method = route.method.toUpperCase() as Method007G;
    setBusy(true);
    const body = {
      source: MARKER007G,
  ...
  const photos = photoFields007G(selected);
  ...
  const sourceHealth = useMemo(() => SOURCES007G.map((source) => results.find((result) => result.key === source.key) ?? null), [results]);
  const driverRows = pickFields007G(selected, [/driver/i, /name/i, /phone/i, /kyc/i, /identity/i, /license/i, /region/i]);
  const vehicleRows = pickFields007G(selected, [/vehicle/i, /car/i, /plate/i, /model/i, /color/i, /insurance/i, /registration/i]);
  const safetyRows = pickFields007G(selected, [/risk/i, /fraud/i, /safety/i, /complaint/i, /status/i, /state/i, /score/i]);
  const financeRows = pickFields007G(selected, [/balance/i, /settlement/i, /amount/i, /hold/i, /commission/i, /reward/i, /bonus/i, /gift/i]);
  const dataQuality = [
    { key: "driver", title: tx007G(language, "driverData"), ok: driverRows.length > 0, count: driverRows.length },
    { key: "vehicle", title: tx007G(language, "vehicleData"), ok: vehicleRows.length > 0, count: vehicleRows.length },
    { key: "photos", title: tx007G(language, "documentPhotos"), ok: photos.length > 0, count: photos.length },
    { key: "safety", title: tx007G(language, "safetyData"), ok: safetyRows.length > 0, count: safetyRows.length },
    { key: "finance", title: tx007G(language, "financeData"), ok: financeRows.length > 0, count: financeRows.length },
  ];
  ...
  const sourceCoveragePercent = Math.round((sourceOk / SOURCES007G.length) * 100);
  const routeCoveragePercent = Math.min(100, Math.round((routes.length / Math.max(1, ACTIONS007G.length)) * 100));
  ...
  const selectedComplaintId = firstId007G(selectedComplaint, ["complaintId", "supportCaseId", "disputeId", "tripId", "id"]);
  const complaintEvidenceRows = pickFields007G(selectedComplaint, [/evidence/i, /photo/i, /image/i, /video/i, /audio/i, /message/i, /chat/i, /note/i, /proof/i], 12);
  const complaintContextRows = pickFields007G(selectedComplaint, [/complaint/i, /dispute/i, /support/i, /trip/i, /driver/i, /rider/i, /amount/i, /route/i, /status/i, /state/i], 14);
  const complaintAiRows = pickFields007G(selectedComplaint, [/ai/i, /recommend/i, /risk/i, /severity/i, /score/i, /fraud/i, /safety/i], 10);
  const complaintActions = ACTIONS007G.filter((item) => item.queue === "complaints");
  const complaintActionCoverage = complaintActions.map((item) => ({ action: item, route: findRoute007G(routes, item) }));
  ...
  return <section className="taxi007g" data-admin-ui-taxi-007g={MARKER007G}>
    <header className="taxi007g-hero">
      <div><span>{MARKER007G}</span><h1>{tx007G(language, "title")}</h1><p>{tx007G(language, "subtitle")}</p></div>
      <div className="taxi007g-actions"><button onClick={refreshAll} disabled={busy}>{busy ? "..." : tx007G(language, "refresh")}</button><button onClick={() => void callSource(SOURCES007G[2]).then((result) => { setLast(result); setResults((prev) => [...prev.filter((item) => item.key !== result.key), result]); })}>{tx007G(language, "diagnostics")}</button></div>
    </header>

    <div className="taxi007g-warning">{tx007G(language, "backendOnly")}</div>

    <nav className="taxi007g-process" aria-label="Taxi workflow order">
      {["load", "choose", "review", "decision", "sync"].map((key, index) => <div key={key}><b>{index + 1}</b><span>{tx007G(language, key)}</span></div>)}
    </nav>

    <section className="taxi007g-metrics">
      <article><span>Backend sources</span><b>{sourceOk}/{SOURCES007G.length}</b><small>real endpoints</small></article>
    ...
    <section className="taxi007g-intakeBoard" data-admin-ui-taxi-007g-intake-board="ready">
      <article className="taxi007g-panel taxi007g-intakeSummary">
        <header><h2>Intake quality</h2><p>real rows, evidence and backend route coverage</p></header>
        <div className="taxi007g-scoreDial"><b>{intakeQualityPercent}%</b><span>dossier completeness</span></div>
        <div className="taxi007g-miniBars">
      ...
      <article className="taxi007g-panel taxi007g-operatorSteps" data-admin-ui-taxi-007g-operator-next-steps="ready">
      ...
      <article className="taxi007g-panel taxi007g-blockers" data-admin-ui-taxi-007g-evidence-gate="ready">
        <header><h2>Review blockers</h2><p>nothing is hidden or locally approved</p></header>
        {reviewBlockers.length ? <ul>{reviewBlockers.map((item) => <li key={item}>{item}</li>)}</ul> : <div className="taxi007g-empty">No UI blocker. Backend is still final source of truth.</div>}
      </article>
    </section>

    <section className="taxi007g-complaintCenter" data-admin-ui-taxi-007g-complaint-review-center="ready">
      <article className="taxi007g-panel taxi007g-complaintSummary">
        <header><h2>Complaint review center</h2><p>жалобы, доказательства, риск от Саби ИИ, отработка и решение только через сервер</p></header>
        <div className="taxi007g-scoreDial"><b>{complaintEvidenceScore}%</b><span>complaint evidence readiness</span></div>
        <div className="taxi007g-complaintStats">
      ...
      <article className="taxi007g-panel taxi007g-complaintSteps" data-admin-ui-taxi-007g-complaint-resolution-flow="ready">
      ...
      <article className="taxi007g-panel taxi007g-complaintEvidence" data-admin-ui-taxi-007g-complaint-evidence-board="ready">
        <header><h2>Evidence board</h2><p>только данные, которые вернул сервер</p></header>
        <div className="taxi007g-evidenceGrid">
          <FieldGroup007G title="Complaint / trip context" rows={complaintContextRows} empty="complaint context not returned" />
          <FieldGroup007G title="Evidence / media / notes" rows={complaintEvidenceRows} empty="evidence not returned" />
          <FieldGroup007G title="AI / risk / severity" rows={complaintAiRows} empty="AI/risk recommendation not returned" />
        </div>
      </article>
      <article className="taxi007g-panel taxi007g-complaintRoutes" data-admin-ui-taxi-007g-complaint-action-routes="ready">
        ...
        {complaintReviewBlockers.length ? <ul>{complaintReviewBlockers.map((item) => <li key={item}>{item}</li>)}</ul> : <div className="taxi007g-empty">Complaint UI blockers are clear. Backend response is still final source of truth.</div>}
      </article>
    </section>

    <section className="taxi007g-workTabs">
      {WORK_TABS007G.map((tab) => <button key={tab.key} className={tab.key === activeTab ? "active" : ""} onClick={() => setActiveTab(tab.key)}><b>{tab.title}</b><span>{tab.description}</span></button>)}
    </section>

    <section className="taxi007g-main">
      <aside className="taxi007g-panel taxi007g-queues">
        <header><h2>{currentTab.title}</h2><p>{currentTab.description}</p></header>
        <div className="taxi007g-queueButtons">{visibleQueues.map((queue) => <button key={queue.key} className={queue.key === active.key ? `active tone-${queue.tone}` : `tone-${queue.tone}`} onClick={() => setActiveQueue(queue.key)}><b>{queue.title}</b><span>{queue.rows.length}</span><small>{queue.subtitle}</small></button>)}</div>
        <label className="taxi007g-search"><span>Search real rows</span><input value={queueSearch} onChange={(event) => setQueueSearch(event.target.value)} placeholder="ID, phone, status, region" /></label>
        <div className="taxi007g-recordList">{filteredRows.length ? filteredRows.slice(0, 16).map((row, index) => {
          const id = firstId007G(row, ["id", "applicationId", "driverApplicationId", "driverId", "vehicleId", "tripId", "complaintId", "settlementId"]);
          return <button key={`${active.key}-${id || index}`} className={row === selected ? "selected" : ""} onClick={() => setSelected(row)}><b>{id || `row-${index + 1}`}</b><span>{text007G(row.status ?? row.state ?? row.kind ?? row.type)}</span><small>{text007G(row.name ?? row.fullName ?? row.driverName ?? row.phone ?? row.region ?? row.createdAt)}</small></button>;
        }) : <div className="taxi007g-empty">{queueSearch.trim() ? "No rows match the search. Clear search to see all backend rows." : tx007G(language, "missingRows")}</div>}</div>
      </aside>

      <main className="taxi007g-panel taxi007g-dossier">
        <header><h2>{tx007G(language, "applicationDossier")}</h2><p>{selectedId || tx007G(language, "selectedId")}</p></header>
        <div className="taxi007g-dossierGrid">
          <FieldGroup007G title={tx007G(language, "driverData")} rows={driverRows} empty="driver data not returned" />
          <FieldGroup007G title={tx007G(language, "vehicleData")} rows={vehicleRows} empty="vehicle data not returned" />
          <FieldGroup007G title={tx007G(language, "safetyData")} rows={safetyRows} empty="safety status not returned" />
          <FieldGroup007G title={tx007G(language, "financeData")} rows={financeRows} empty="balance/settlement not returned" />
        </div>
        <section className="taxi007g-docs"><header><h3>{tx007G(language, "documentPhotos")}</h3><span>{photos.length}</span></header>{photos.length ? <div>{photos.map(([key, url]) => <a key={key} href={url} target="_blank" rel="noreferrer"><span>{key}</span><b>open</b></a>)}</div> : <em>{tx007G(language, "missingPhotos")}</em>}</section>
      </main>

      <aside className="taxi007g-panel taxi007g-decision">
        <header><h2>{tx007G(language, "decisionPanel")}</h2><p>{action.title}</p></header>
        <div className="taxi007g-actionList">{actions.map((item) => <button key={item.key} className={item.key === action.key ? `active tone-${item.tone}` : `tone-${item.tone}`} onClick={() => setActionKey(item.key)}><b>{item.title}</b><span>{item.description}</span></button>)}</div>
        <div className="taxi007g-checklist">{active.requiredReview.map((item) => <label key={item}><input type="checkbox" checked={checked[`${active.key}:${item}`] === true} onChange={(event) => setChecked((prev) => ({ ...prev, [`${active.key}:${item}`]: event.target.checked }))} /><span>{item}</span></label>)}</div>
        <label><span>{tx007G(language, "selectedId")}</span><input value={manualId || selectedId} onChange={(event) => setManualId(event.target.value)} placeholder="real backend id" /></label>
        <label><span>{tx007G(language, "reason")}</span><textarea value={adminReason} onChange={(event) => setAdminReason(event.target.value)} placeholder="required admin reason" /></label>
        <label><span>{tx007G(language, "ownerApproval")}</span><input value={ownerApprovalId} onChange={(event) => setOwnerApprovalId(event.target.value)} placeholder="required for critical action" /></label>
        <div className="taxi007g-route"><span>Backend route</span><b>{route?.method ?? DASH} {route?.path ?? "route contract not found"}</b><small>{route?.safeDisabledUntilNextStage ? "safe-disabled by backend" : "backend response is source of truth"}</small></div>
        <button className="taxi007g-run" disabled={!canRun} onClick={runAction}>{canRun ? tx007G(language, "runAction") : tx007G(language, "blocked")}</button>
    ...
    <section className="taxi007g-truthGrid" data-admin-ui-taxi-007g-data-truth="ready">
      <article className="taxi007g-panel"><header><h2>{tx007G(language, "dataTruth")}</h2><p>selected dossier completeness from backend payload only</p></header><div className="taxi007g-qualityList">{dataQuality.map((item) => <div key={item.key} className={item.ok ? "ok" : "bad"}><b>{item.title}</b><span>{item.ok ? "returned" : "missing"}</span><strong>{item.count}</strong></div>)}</div><p className="taxi007g-note">{loadedRows ? "Rows are parsed from backend payloads. Missing evidence stays missing." : tx007G(language, "emptyStateFix") + ": Taxi read-only DB rows endpoint must return queue rows."}</p></article>
      <article className="taxi007g-panel"><header><h2>{tx007G(language, "sourceHealth")}</h2><button onClick={() => setSourceOpen((value) => !value)}>{sourceOpen ? "hide" : "show"}</button></header><div className="taxi007g-sourceHealth">{sourceHealth.map((item, index) => { const source = SOURCES007G[index]; return <button key={source.key} className={item?.ok ? "ok" : "bad"} onClick={() => void callSource(source).then((result) => { setLast(result); setResults((prev) => [...prev.filter((old) => old.key !== result.key), result]); })}><b>{source.title}</b><span>{item ? `HTTP ${item.status}` : "not loaded"}</span><small>{source.method} {source.path}</small></button>; })}</div>{sourceOpen ? <pre>{JSON.stringify(sourceHealth.map((item, index) => ({ source: SOURCES007G[index].key, status: item?.status ?? null, ok: item?.ok ?? false, message: item?.message ?? "not loaded" })), null, 2)}</pre> : null}</article>
      <article className="taxi007g-panel"><header><h2>{tx007G(language, "actionJournal")}</h2><p>backend-only attempts</p></header>{actionJournal.length ? <div className="taxi007g-journal">{actionJournal.map((item) => <div key={item.id} className={item.ok ? "ok" : "bad"}><b>{item.title}</b><span>{item.queue} · {item.recordId || DASH}</span><small>HTTP {item.status} · {item.message} · sync {item.synced ? "done" : "not changed"}</small></div>)}</div> : <div className="taxi007g-empty">No backend action attempted in this session.</div>}</article>
    </section>

    <section className="taxi007g-bottom">
      <article className="taxi007g-panel"><header><h2>{tx007G(language, "reports")}</h2><p>queues, route coverage, locked reasons</p></header>{queues.map((queue) => <div className="taxi007g-bar" key={queue.key}><span>{queue.title}</span><b style={{ width: `${Math.min(100, queue.rows.length * 10 + 8)}%` }} /><strong>{queue.rows.length}</strong></div>)}</article>
      <article className="taxi007g-panel"><header><h2>{tx007G(language, "latest")}</h2><p>{last ? `HTTP ${last.status} · ${last.ms}ms` : DASH}</p></header>{last ? <div className={last.ok ? "taxi007g-response ok" : "taxi007g-response bad"}><b>{last.title}</b><span>{last.method} {last.path}</span><small>{last.message}</small><pre>{JSON.stringify(last.payload, null, 2)}</pre></div> : <div className="taxi007g-empty">{tx007G(language, "missingRows")}</div>}</article>
    </section>

    <section className="taxi007g-panel taxi007g-sources"><header><h2>{tx007G(language, "sources")}</h2><button onClick={() => setTechnicalOpen((value) => !value)}>{technicalOpen ? "hide" : "show"}</button></header><div>{results.map((item) => <button key={item.key} className={item.ok ? "ok" : "bad"} onClick={() => setLast(item)}><b>{item.title}</b><span>HTTP {item.status}</span><small>{item.path}</small></button>)}</div>{technicalOpen ? <pre>{JSON.stringify({ routes: routes.slice(0, 100), results: results.map(({ key, status, ok, path, ms }) => ({ key, status, ok, path, ms })) }, null, 2)}</pre> : null}</section>
  </section>;
}
