...
type Props007V = { language: AdminLanguage; config: AdminApiConfig; setNotice: (message: string) => void };
type Tab007V = "applications" | "drivers" | "vehicles" | "tariffs" | "orders" | "trips" | "complaints" | "balance" | "rewards" | "contests" | "archive" | "reports" | "access";
type Field007V = { key: string; label: string; type?: "text" | "number" | "textarea" | "select"; required?: boolean; options?: string[]; placeholder?: string };
type Action007V = { key: string; label: string; path: string; tone: "ready" | "warn" | "danger" | "locked"; ownerRequired?: boolean; checksRequired?: string[] };
type Workflow007V = { key: string; tab: Tab007V; title: string; subtitle: string; routeBase: string; recordLabel: string; fields: Field007V[]; checks: string[]; actions: Action007V[] };
type LastResponse007V = { ok: boolean; status: number | string; action: string; route: string; message: string; payload?: unknown; createdAt: string } | null;
type SourceStatus007V = { path: string; ok: boolean; status: number | string; message: string; at: string };
type Copy007V = Record<string, string>;

const MARKER007V = "ADMIN-UI-TAXI-007V-APPLICATIONS-100-WORKFLOW";
const READ_ONLY_BACKEND_NOTE_007V = "read-only backend";

const COPY007V: Record<AdminLanguage, Copy007V> = {
  ru: {
    ...
    stage: "Шаг 19 · заявки 100% внутри полной консоли такси",
  ...
  en: {
    ...
    stage: "Step 19 · applications workflow 100% inside full Taksi console",
  ...
  uz: {
    ...
    stage: "19 · arizalar 100% va to‘liq taksi konsoli",
  ...
  zh: {
    ...
    stage: "步骤 19 · 申请流程 100% 保留完整出租车控制台",
...
const TAB_KEYS007V: Tab007V[] = ["applications", "drivers", "vehicles", "tariffs", "orders", "trips", "complaints", "balance", "rewards", "contests", "archive", "reports", "access"];
const requiredCommonFields007T = ["recordId", "adminReason"];

function workflows007V(copy: Copy007V): Workflow007V[] {
  return [
    {
      ...
      fields: [
        ...
        { key: "applicationNumber", label: "Номер заявки", required: true },
        { key: "applicationSource", label: "Источник заявки", type: "select", options: ["mobile_app", "web", "support", "partner", "manager_manual"], required: true },
        { key: "assignedManagerId", label: "Номер менеджера такси", required: true },
        { key: "reviewShiftId", label: "Номер смены проверки", required: true },
        ...
        { key: "district", label: "Район / город", required: true },
        { key: "submittedAt", label: "Дата подачи заявки", required: true },
        { key: "dateOfBirth", label: "Дата рождения", required: true },
        { key: "driverExperienceYears", label: "Стаж водителя", type: "number", required: true },
        { key: "emergencyContact", label: "Экстренный контакт", required: true },
        { key: "kycStatus", label: "Статус проверки личности", type: "select", options: ["pending", "in_review", "approved", "rejected"], required: true },
        { key: "phoneVerified", label: "Телефон подтверждён", type: "select", options: ["yes", "no"], required: true },
        { key: "passportId", label: "Паспорт / номер документа", required: true },
        { key: "passportExpiry", label: "Срок паспорта / документа", required: true },
        ...
        { key: "licenseExpiry", label: "Срок водительских прав", required: true },
        { key: "licensePhotoUrl", label: "Фото водительских прав", required: true },
        { key: "taxiPermitNumber", label: "Разрешение такси", required: true },
        { key: "taxiPermitExpiry", label: "Срок разрешения такси", required: true },
        { key: "insuranceDocumentUrl", label: "Страховка", required: true },
        { key: "insuranceExpiry", label: "Срок страховки", required: true },
        { key: "registrationDocumentUrl", label: "Регистрация авто", required: true },
        { key: "registrationExpiry", label: "Срок регистрации авто", required: true },
        { key: "technicalInspectionUrl", label: "Техосмотр", required: true },
        { key: "technicalInspectionExpiry", label: "Срок техосмотра", required: true },
        { key: "vehiclePlate", label: "Госномер авто", required: true },
        { key: "vehicleVin", label: "Номер кузова", required: true },
        ...
        { key: "vehicleSeatCount", label: "Количество мест", type: "number", required: true },
        ...
        { key: "vehicleTrunkPhotoUrl", label: "Фото багажника", required: true },
        ...
        { key: "blacklistCheck", label: "Проверка чёрного списка", type: "select", options: ["clear", "review", "blocked"], required: true },
        { key: "complaintHistoryCheck", label: "История жалоб", type: "select", options: ["clear", "review", "critical"], required: true },
        { key: "safetyTrainingStatus", label: "Инструктаж безопасности", type: "select", options: ["not_started", "in_review", "passed", "failed"], required: true },
        { key: "documentMismatchReason", label: "Несовпадения документов", type: "textarea" },
        { key: "missingDocumentsList", label: "Недостающие документы", type: "textarea" },
        { key: "rejectReason", label: "Причина отказа", type: "textarea" },
        { key: "archiveFolderId", label: "Номер архива проверки", required: true },
        { key: "archiveEvidenceLinks", label: "Ссылки архива доказательств", type: "textarea", required: true },
      ...
      checks: ["applicationSourceChecked", "menejerAssignedChecked", "identityChecked", "ageAndRegionChecked", "phoneVerifiedChecked", "profilePhotoChecked", "selfiePhotoChecked", "passportExpiryChecked", "licensePhotoChecked", "licenseExpiryChecked", "taxiPermitChecked", "vehicleExteriorPhotoChecked", "vehicleInteriorPhotoChecked", "vehicleTrunkPhotoChecked", "registrationChecked", "registrationExpiryChecked", "insuranceChecked", "insuranceExpiryChecked", "technicalInspectionChecked", "plateMatchesVehicleChecked", "vinChecked", "vehicleCategoryChecked", "kycStatusChecked", "duplicateDriverChecked", "blacklistChecked", "complaintHistoryChecked", "fraudRiskChecked", "safetyTrainingChecked", "ownerSabiAiReviewed", "menejerCommentReady", "archiveReady", "ownerCommandAttached"],
      actions: [
        { key: "approve", label: "Подтвердить заявку", path: "/approve", tone: "ready", checksRequired: ["applicationSourceChecked", "managerAssignedChecked", "identityChecked", "ageAndRegionChecked", "phoneVerifiedChecked", "profilePhotoChecked", "selfiePhotoChecked", "passportExpiryChecked", "licensePhotoChecked", "licenseExpiryChecked", "taxiPermitChecked", "vehicleExteriorPhotoChecked", "vehicleInteriorPhotoChecked", "registrationChecked", "insuranceChecked", "technicalInspectionChecked", "plateMatchesVehicleChecked", "vinChecked", "kycStatusChecked", "duplicateDriverChecked", "blacklistChecked", "fraudRiskChecked", "safetyTrainingChecked", "ownerSabiAiReviewed", "archiveReady"] },
        { key: "reject", label: "Отклонить заявку", path: "/reject", tone: "danger", checksRequired: ["identityChecked", "blacklistChecked", "fraudRiskChecked", "ownerSabiAiReviewed", "managerCommentReady", "archiveReady"] },
        ...
        { key: "request-vehicle-inspection", label: "Запросить осмотр авто", path: "/request-vehicle-inspection", tone: "warn", checksRequired: ["vehicleExteriorPhotoChecked", "managerCommentReady"] },
        { key: "send-risk-review", label: "Отправить на риск-проверку", path: "/risk-review", tone: "locked", checksRequired: ["fraudRiskChecked", "ownerSabiAiReviewed", "managerCommentReady"] },
...
function emptyForm007V(workflow: Workflow007V): Record<string, string> {
...
function allRequiredReady007V(workflow: Workflow007V, form: Record<string, string>, checks: Record<string, boolean>, action: Action007V): boolean {
...
function buildUrl007V(baseUrl: string, path: string): string {
  const base = baseUrl.replace(/\/$/, "");
  const clean = path.startsWith("/") ? path : `/${path}`;
  return `${base}${clean}`;
}

async function readJsonSafe007V(response: Response): Promise<unknown> {
  const text = await response.text();
  if (!text) return null;
  try { return JSON.parse(text); } catch { return { raw: text }; }
}

function collectRows007V(value: unknown, out: Record<string, unknown>[] = []): Record<string, unknown>[] {
  if (Array.isArray(value)) {
    value.forEach((item) => { if (item && typeof item === "object" && !Array.isArray(item)) out.push(item as Record<string, unknown>); else collectRows007V(item, out); });
  } else if (value && typeof value === "object") {
    Object.values(value as Record<string, unknown>).forEach((next) => collectRows007V(next, out));
  }
  return out.slice(0, 80);
}

function rowValue007V(row: Record<string, unknown>, keys: string[]): string {
...
function buildBlockers007V(workflow: Workflow007V, form: Record<string, string>, checks: Record<string, boolean>, action: Action007V): string[] {
...
export function TaxiAdminControl007VPanel({ language, config, setNotice }: Props007V) {
  const copy = COPY007V[language] || COPY007V.ru;
  const workflows = useMemo(() => workflows007V(copy), [copy]);
  const [activeTab, setActiveTab] = useState<Tab007V>("applications");
  ...
  const [form, setForm] = useState<Record<string, string>>(() => emptyForm007V(selected));
  const [checks, setChecks] = useState<Record<string, boolean>>({});
  const [last, setLast] = useState<LastResponse007V>(null);
  const [sources, setSources] = useState<SourceStatus007V[]>([]);
  ...
  const fillFromLiveRow = (row: Record<string, unknown>) => {
    ...
    setForm((prev) => ({
      ...prev,
      recordId: rowValue007V(row, ["id", "applicationId", "driverId", "vehicleId", "tripId", "requestId", "complaintId", "tariffId", "ledgerId"]) || prev.recordId || "",
      driverId: rowValue007V(row, ["driverId", "driver_id", "driver"] ) || prev.driverId || "",
      riderId: rowValue007V(row, ["riderId", "rider_id", "rider"] ) || prev.riderId || "",
      region: rowValue007V(row, ["region", "zone", "city"] ) || prev.region || "",
      phone: rowValue007V(row, ["phone", "driverPhone", "mobile"] ) || prev.phone || "",
      vehiclePlate: rowValue007V(row, ["vehiclePlate", "plate", "licensePlate"] ) || prev.vehiclePlate || "",
      plate: rowValue007V(row, ["plate", "vehiclePlate", "licensePlate"] ) || prev.plate || "",
      vehicleModel: rowValue007V(row, ["vehicleModel", "model", "carModel"] ) || prev.vehicleModel || "",
      model: rowValue007V(row, ["model", "vehicleModel", "carModel"] ) || prev.model || "",
      submittedAt: rowValue007V(row, ["submittedAt", "createdAt", "created_at"] ) || prev.submittedAt || "",
      kycStatus: rowValue007V(row, ["kycStatus", "kyc_status", "verificationStatus"] ) || prev.kycStatus || "",
      passportId: rowValue007V(row, ["passportId", "documentId", "idDocument"] ) || prev.passportId || "",
      profilePhotoUrl: rowValue007V(row, ["profilePhotoUrl", "driverPhotoUrl", "avatarUrl"] ) || prev.profilePhotoUrl || "",
      selfieUrl: rowValue007V(row, ["selfieUrl", "selfiePhotoUrl", "identitySelfieUrl"] ) || prev.selfieUrl || "",
      licensePhotoUrl: rowValue007V(row, ["licensePhotoUrl", "driverLicensePhotoUrl"] ) || prev.licensePhotoUrl || "",
      insuranceDocumentUrl: rowValue007V(row, ["insuranceDocumentUrl", "insurancePhotoUrl"] ) || prev.insuranceDocumentUrl || "",
      registrationDocumentUrl: rowValue007V(row, ["registrationDocumentUrl", "vehicleRegistrationUrl"] ) || prev.registrationDocumentUrl || "",
      vehicleYear: rowValue007V(row, ["vehicleYear", "year", "carYear"] ) || prev.vehicleYear || "",
      vehicleColor: rowValue007V(row, ["vehicleColor", "color", "carColor"] ) || prev.vehicleColor || "",
      vehicleCategory: rowValue007V(row, ["vehicleCategory", "category", "class"] ) || prev.vehicleCategory || "",
      vehicleExteriorPhotoUrl: rowValue007V(row, ["vehicleExteriorPhotoUrl", "carExteriorPhotoUrl"] ) || prev.vehicleExteriorPhotoUrl || "",
      vehicleInteriorPhotoUrl: rowValue007V(row, ["vehicleInteriorPhotoUrl", "carInteriorPhotoUrl"] ) || prev.vehicleInteriorPhotoUrl || "",
      fraudScore: rowValue007V(row, ["fraudScore", "riskScore"] ) || prev.fraudScore || "",
      duplicateCheck: rowValue007V(row, ["duplicateCheck", "duplicateStatus"] ) || prev.duplicateCheck || "",
      archiveFolderId: rowValue007V(row, ["archiveFolderId", "archiveId"] ) || prev.archiveFolderId || "",
    }));
  };
  const requestedOperationRoute = (action: Action007V) => `${selected.routeBase}/${encodeURIComponent(form.recordId || "record")}${action.path}`;
  const actionRoute = (_action: Action007V) => form.routeOverride.trim() || "/api/taxi/002x/db-write-runtime/write-gate";
  ...
  const syncSources = async () => {
    ...
    const result: SourceStatus007V[] = [];
    for (const path of paths) {
      try {
        const response = await fetch(buildUrl007V(config.baseUrl, path), { headers: { "x-sabi-taxi-db-dry-run": "read-only-approved-002t" } });
        const data = await readJsonSafe007V(response);
        if (path.includes("read-only-db-dry-run") && !path.endsWith("/plan")) {
          const rows = collectRows007V(data);
          setLiveRows(rows);
  ...
  const submitAction = async (action: Action007V) => {
    const blockers = buildBlockers007V(selected, form, checks, action);
    ...
    const payload = {
      source: "admin-ui-007v-applications-full-console",
      marker: MARKER007V,
    ...
    try {
      const response = await fetch(buildUrl007V(config.baseUrl, route), {
        method: "POST",
  ...
  const activeBlockers = selected.actions.flatMap((action) => buildBlockers007V(selected, form, checks, action));
  ...
  const readyActions = selected.actions.filter((action) => allRequiredReady007V(selected, form, checks, action));
  ...
  const applicationFieldGroups = [
    { key: "intake", title: "1. Приём заявки", detail: "Источник, менеджер, смена, телефон, регион и статус проверки личности.", fields: ["recordId", "applicationNumber", "applicationSource", "assignedManagerId", "reviewShiftId", "driverName", "phone", "region", "district", "submittedAt", "dateOfBirth", "driverExperienceYears", "emergencyContact", "kycStatus", "phoneVerified"] },
    { key: "identity", title: "2. Личность и фото", detail: "Паспорт или номер документа, селфи, профильное фото и сроки документов.", fields: ["passportId", "passportExpiry", "profilePhotoUrl", "selfieUrl", "licenseNumber", "licenseExpiry", "licensePhotoUrl", "taxiPermitNumber", "taxiPermitExpiry"] },
    { key: "vehicleDocs", title: "3. Документы авто", detail: "Страховка, регистрация, техосмотр и сроки действия.", fields: ["insuranceDocumentUrl", "insuranceExpiry", "registrationDocumentUrl", "registrationExpiry", "technicalInspectionUrl", "technicalInspectionExpiry"] },
    { key: "vehicle", title: "4. Авто и фотографии", detail: "Госномер, номер кузова, модель, категория, фото кузова, салона и багажника.", fields: ["vehiclePlate", "vehicleVin", "vehicleModel", "vehicleYear", "vehicleColor", "vehicleCategory", "vehicleSeatCount", "vehicleExteriorPhotoUrl", "vehicleInteriorPhotoUrl", "vehicleTrunkPhotoUrl"] },
    { key: "risk", title: "5. Риск и честность", detail: "Дубли, чёрный список, жалобы, оценка мошенничества и инструктаж.", fields: ["fraudScore", "duplicateCheck", "blacklistCheck", "complaintHistoryCheck", "safetyTrainingStatus", "documentMismatchReason", "missingDocumentsList", "rejectReason"] },
    { key: "decision", title: "6. Решение и архив", detail: "Архив, Саби ИИ владельца, комментарий менеджера и команда владельца.", fields: ["archiveFolderId", "archiveEvidenceLinks", "ownerSabiAiNote", "menejerComment"] },
  ];
  const renderField = (field: Field007V) => (
  ...
  return (
    <section className="messengerStyle007B taxi007vPremium" data-admin-ui-taxi-007v-applications-full-console="ready" data-admin-ui-taxi-007v-no-fake-success="ready">
      <div className="ms007b-hero taxi007vHeroCompact">
        <div>
        ...
        <div className="ms007b-heroBadges">
          ...
          <span>{MARKER007V}</span>
        </div>
      </div>

      <div className="taxi007vCommandBar" data-admin-ui-taxi-007t-fix1-command-bar="ready">
        <button type="button" className="taxi007vPrimary" onClick={() => void syncSources()}>{copy.loadData}</button>
        ...
        <div className="taxi007vCommandStatus"><strong>{readyActions.length}/{selected.actions.length}</strong><span>{readyActions.length ? copy.ready : copy.blocked}</span></div>
      </div>

      <div className="ms007b-tabs taxi007vStickyTabs" data-admin-ui-taxi-007t-tabs="ready">
        {TAB_KEYS007V.map((tab) => (
      ...
      <div className="ms007b-metrics taxi007vMetrics">
      ...
      <div className="taxi007vWorkspace" data-admin-ui-taxi-007t-fix1-premium-workspace="ready">
        <aside className="taxi007vQueuePane">
          <div className="ms007b-sectionTitle"><h2>{copy.selectedWorkflow}</h2><span>{visibleWorkflows.length}</span></div>
          <div className="taxi007vWorkflowList" data-admin-ui-taxi-007t-workflow-list="ready">
          ...
          <div className="taxi007vLiveMini" data-admin-ui-taxi-007t-live-rows="ready">
            ...
            {currentRows.length ? currentRows.map((row, index) => (
              <button key={`${rowValue007V(row, ["id", "driverId", "applicationId", "tripId"]) || index}`} type="button" onClick={() => fillFromLiveRow(row)}>
                <strong>{rowValue007V(row, ["id", "applicationId", "driverId", "vehicleId", "tripId", "complaintId"]) || `row-${index + 1}`}</strong>
                <span>{rowValue007V(row, ["status", "state", "kind", "type", "region"]) || "read-only"}</span>
              </button>
            )) : <div className="taxi007vEmpty">{copy.loadData}</div>}
          </div>
        </aside>

        <main className="taxi007vFormPane" data-admin-ui-taxi-007t-action-form="ready">
          <div className="taxi007vPanelHeader">
          ...
          <div className="taxi007vRequiredStrip">
          ...
          {selected.tab === "applications" ? (
            <div className="taxi007vApplicationBoard" data-admin-ui-taxi-007v-applications-100-board="ready">
              <div className="taxi007vApplicationCard">
                <h3>1. Водитель</h3>
              ...
              <div className="taxi007vApplicationCard owner">
          ...
          {selected.tab === "applications" ? (
            <div className="taxi007vApplicationSections" data-admin-ui-taxi-007v-application-sections="ready">
              {applicationFieldGroups.map((group) => {
                const groupFields = selected.fields.filter((field) => group.fields.includes(field.key));
                const complete = groupFields.filter((field) => String(form[field.key] || "").trim()).length;
                const required = groupFields.filter((field) => field.required).length;
                const completeRequired = groupFields.filter((field) => field.required && String(form[field.key] || "").trim()).length;
                return (
                  <section key={group.key} className="taxi007vApplicationSection">
                    <div className="taxi007vApplicationSectionHead">
                      <div><h3>{group.title}</h3><p>{group.detail}</p></div>
                      <strong>{completeRequired}/{required}</strong>
                    </div>
                    <div className="taxi007vFormGridPremium">
                      {groupFields.map(renderField)}
                    </div>
                    <small>{complete}/{groupFields.length} fields filled · backend-only</small>
                  </section>
                );
              })}
              <section className="taxi007vApplicationSection decision">
                <div className="taxi007vApplicationSectionHead">
                  <div><h3>7. Шлюз владельца и маршрут</h3><p>Финальное действие не меняет статус локально. Отправляется серверный пакет и ожидается ответ сервера.</p></div>
                  <strong>{String(form.ownerApprovalId || "").trim() ? "owner" : "gate"}</strong>
                </div>
                <div className="taxi007vFormGridPremium">
            ...
            <div className="taxi007vFormGridPremium" data-admin-ui-taxi-007t-form-fields="ready">
          ...
          <div className="taxi007vChecksPremium" data-admin-ui-taxi-007t-review-checks="ready">
        ...
        <aside className="taxi007vActionDock" data-admin-ui-taxi-007t-action-buttons="ready">
          <div className="taxi007vDockTop">
            <h2>{copy.actionButtons}</h2>
            <p>{copy.noFake}</p>
          </div>
          <div className="taxi007vActionButtons">
            {selected.actions.map((action) => {
              const blockers = buildBlockers007V(selected, form, checks, action);
              const ready = blockers.length === 0;
              return (
                <button key={action.key} type="button" className={`taxi007vAction ${action.tone} ${ready ? "ready" : "blocked"}`} disabled={busyAction === action.key} onClick={() => void submitAction(action)}>
          ...
          <div className="taxi007vBlockersPremium" data-admin-ui-taxi-007t-blockers="ready">
            <h3>{copy.blocked}</h3>
            {uniqueBlockers.length ? uniqueBlockers.slice(0, 10).map((blocker) => <span key={blocker}>{blocker}</span>) : <b>{copy.ready}</b>}
          </div>
          <div className="taxi007vAiBox"><strong>Owner Sabi AI</strong><span>{copy.aiGuard}</span></div>
        </aside>
      </div>

      <div className="taxi007vBottomGrid" data-admin-ui-taxi-007t-sync-archive="ready">
        ...
        <section className="ms007b-mainCard">
          ...
          <details className="taxi007vDetails"><summary>Payload / sync package</summary><pre>{JSON.stringify({ last, selectedWorkflow: selected.key, selectedLiveRow, form, checked: Object.keys(checks).filter((key) => checks[key]) }, null, 2)}</pre></details>
        </section>
      </div>
    </section>
  );
