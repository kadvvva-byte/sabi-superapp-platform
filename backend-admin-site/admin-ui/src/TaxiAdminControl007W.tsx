...
type Props007W = { language: AdminLanguage; config: AdminApiConfig; setNotice: (message: string) => void };
type Tab007W = "applications" | "drivers" | "vehicles" | "tariffs" | "orders" | "trips" | "complaints" | "balance" | "rewards" | "contests" | "archive" | "reports" | "access";
type Category007W = "standard" | "comfort" | "business" | "delivery" | "intercity";
type Action007W = "approve" | "reject" | "request-docs" | "archive";
type LastResponse007W = { action: string; ok: boolean; status: number | string; message: string; route: string; at: string } | null;

type TaxiApplication007W = {
  applicationId: string;
  category: Category007W;
  status: string;
  submittedAt: string;
  driverName: string;
  phone: string;
...
type Copy007W = {
  ...
  reports: string;
  access: string;
  categories: Record<Category007W, string>;
...
const MARKER007W_FIX1 = "ADMIN-UI-TAXI-007W-FIX1-APPLICATIONS-LIST-OPEN-APPROVE-ARCHIVE";
const WRITE_GATE_007W = "/api/taxi/002x/db-write-runtime/write-gate";
const READ_ONLY_007W = "/api/taxi/002t/read-only-db-dry-run";

const TABS007W: Tab007W[] = ["applications", "drivers", "vehicles", "tariffs", "orders", "trips", "complaints", "balance", "rewards", "contests", "archive", "reports", "access"];
const CATEGORIES007W: Category007W[] = ["standard", "comfort", "business", "delivery", "intercity"];

const COPY007W: Record<AdminLanguage, Copy007W> = {
  ru: {
    title: "Такси: заявки водителей",
    subtitle: "Простой рабочий экран: список новых заявок, открыть заявку, посмотреть документы и фото из мобильной загрузки, утвердить или отклонить, обязательно сохранить архив. При утверждении готовится запись в базу утверждённых водителей через серверный шлюз.",
...
const emptyApplication007W: TaxiApplication007W = {
  applicationId: "",
  category: "standard",
  status: "new",
  submittedAt: "",
...
const normalizeBase007W = (config: AdminApiConfig): string => String(config.baseUrl || "http://127.0.0.1:3000").replace(/\/$/, "");

function pick007W(row: Record<string, unknown>, keys: string[]): string {
...
function normalizeCategory007W(value: string): Category007W {
...
function fromLiveRow007W(row: Record<string, unknown>): TaxiApplication007W {
  return {
    applicationId: pick007W(row, ["applicationId", "driverApplicationId", "id"]),
    category: normalizeCategory007W(pick007W(row, ["category", "serviceCategory", "tariffCategory", "requestedCategory"])),
    status: pick007W(row, ["status", "applicationStatus"]) || "new",
    submittedAt: pick007W(row, ["submittedAt", "createdAt", "receivedAt"]),
    driverName: pick007W(row, ["driverName", "fullName", "name"]),
    phone: pick007W(row, ["phone", "phoneNumber", "driverPhone"]),
    region: pick007W(row, ["region", "city", "serviceRegion"]),
    driverPhotoUrl: pick007W(row, ["driverPhotoUrl", "profilePhotoUrl", "photoUrl"]),
    passportUrl: pick007W(row, ["passportUrl", "passportPhotoUrl", "idDocumentUrl"]),
    licenseUrl: pick007W(row, ["licenseUrl", "licensePhotoUrl", "driverLicenseUrl"]),
    vehicleDocumentUrl: pick007W(row, ["vehicleDocumentUrl", "registrationUrl", "registrationDocumentUrl", "vehicleRegistrationUrl"]),
    vehiclePlate: pick007W(row, ["vehiclePlate", "plate", "plateNumber"]),
    vehicleModel: pick007W(row, ["vehicleModel", "model", "carModel"]),
    vehicleFrontPhotoUrl: pick007W(row, ["vehicleFrontPhotoUrl", "carFrontPhotoUrl", "frontPhotoUrl"]),
    vehicleBackPhotoUrl: pick007W(row, ["vehicleBackPhotoUrl", "carBackPhotoUrl", "backPhotoUrl"]),
    vehicleLeftPhotoUrl: pick007W(row, ["vehicleLeftPhotoUrl", "carLeftPhotoUrl", "leftPhotoUrl", "vehicleSidePhotoUrl"]),
    vehicleRightPhotoUrl: pick007W(row, ["vehicleRightPhotoUrl", "carRightPhotoUrl", "rightPhotoUrl"]),
    interiorFrontPhotoUrl: pick007W(row, ["interiorFrontPhotoUrl", "frontInteriorPhotoUrl"]),
    interiorBackPhotoUrl: pick007W(row, ["interiorBackPhotoUrl", "backInteriorPhotoUrl", "vehicleInteriorPhotoUrl"]),
    dashboardPhotoUrl: pick007W(row, ["dashboardPhotoUrl", "instrumentPanelPhotoUrl", "vehicleDashboardPhotoUrl"]),
    mileagePhotoUrl: pick007W(row, ["mileagePhotoUrl", "odometerPhotoUrl", "vehicleMileagePhotoUrl"]),
    rawMobileUploadId: pick007W(row, ["mobileUploadId", "uploadId", "rawMobileUploadId"]),
  };
}

const requiredApproveFields007W: Array<keyof TaxiApplication007W> = ["applicationId", "category", "driverName", "phone", "region", "driverPhotoUrl", "passportUrl", "licenseUrl", "vehicleDocumentUrl", "vehiclePlate", "vehicleModel", "vehicleFrontPhotoUrl", "vehicleBackPhotoUrl", "vehicleLeftPhotoUrl", "vehicleRightPhotoUrl", "interiorFrontPhotoUrl", "interiorBackPhotoUrl", "dashboardPhotoUrl", "mileagePhotoUrl"];

type Checks007W = { driverDocs: boolean; vehicleDocs: boolean; vehiclePhotos: boolean; archive: boolean };

function blockers007W(application: TaxiApplication007W | null, checks: Checks007W, action: Action007W, reason: string, archiveId: string, copy: Copy007W): string[] {
  const blockers: string[] = [];
  if (!application?.applicationId) blockers.push(`${copy.appId}: ${copy.required}`);
  if (action === "approve") {
    requiredApproveFields007W.forEach((key) => {
...
function readiness007W(application: TaxiApplication007W | null, checks: Checks007W, archiveId: string): number {
  if (!application?.applicationId) return 0;
  const filled = requiredApproveFields007W.filter((key) => String(application[key] || "").trim()).length;
  const checked = Object.values(checks).filter(Boolean).length;
  const archive = archiveId.trim() ? 1 : 0;
  return Math.round(((filled + checked + archive) / (requiredApproveFields007W.length + 5)) * 100);
}

function dataPairs007W(copy: Copy007W, application: TaxiApplication007W): Array<[string, string]> {
...
function vehiclePairs007W(copy: Copy007W, application: TaxiApplication007W): Array<[string, string]> {
  return [[copy.plate, application.vehiclePlate], [copy.model, application.vehicleModel]];
}

function docPairs007W(copy: Copy007W, application: TaxiApplication007W): Array<[string, string]> {
...
function photoPairs007W(copy: Copy007W, application: TaxiApplication007W): Array<[string, string]> {
...
function buildPayload007W(action: Action007W, application: TaxiApplication007W | null, checks: Checks007W, decisionReason: string, archiveId: string, ownerNote: string, route: string) {
  return {
    source: "admin-ui-007w-fix1-applications-list-open-approve-archive",
    action,
...
export function TaxiAdminControl007WPanel({ language, config, setNotice }: Props007W) {
  const copy = COPY007W[language] || COPY007W.ru;
  const [activeTab, setActiveTab] = useState<Tab007W>("applications");
  const [activeCategory, setActiveCategory] = useState<Category007W>("standard");
  const [applications, setApplications] = useState<TaxiApplication007W[]>([]);
  const [selectedId, setSelectedId] = useState("");
  const [checks, setChecks] = useState<Checks007W>({ driverDocs: false, vehicleDocs: false, vehiclePhotos: false, archive: false });
  const [decisionReason, setDecisionReason] = useState("");
  const [archiveId, setArchiveId] = useState("");
  const [ownerNote, setOwnerNote] = useState("");
  const [routeOverride, setRouteOverride] = useState("");
  const [lastResponse, setLastResponse] = useState<LastResponse007W>(null);
  const [busyAction, setBusyAction] = useState<string>("");

  const route = routeOverride.trim() || WRITE_GATE_007W;
  ...
  const readinessPercent = readiness007W(selectedApplication, checks, archiveId);
  const productionReadinessPercent = 74;
  const approveBlockers = useMemo(() => blockers007W(selectedApplication, checks, "approve", decisionReason, archiveId, copy), [selectedApplication, checks, decisionReason, archiveId, copy]);
  ...
  const loadApplications = async () => {
    try {
      const response = await fetch(`${normalizeBase007W(config)}${READ_ONLY_007W}`, { headers: { "x-sabi-admin-token": config.token || "", "x-sabi-taxi-db-dry-run": "read-only-approved-002t" } });
      const json = await response.json().catch(() => ({}));
      const rows = Array.isArray(json?.rows) ? json.rows : Array.isArray(json?.data?.rows) ? json.data.rows : [];
      const mapped = rows.map((row: Record<string, unknown>) => fromLiveRow007W(row)).filter((item: TaxiApplication007W) => item.applicationId && (item.status.toLowerCase().includes("new") || item.status.toLowerCase().includes("pending") || item.status.toLowerCase().includes("review")));
      setApplications(mapped);
      setSelectedId(mapped[0]?.applicationId || "");
      resetReviewState();
      setLastResponse({ action: copy.load, ok: response.ok, status: response.status, route: READ_ONLY_007W, message: mapped.length ? `${mapped.length}` : "no_new_applications_from_backend", at: new Date().toISOString() });
      setNotice(response.ok ? copy.refresh : copy.blocked);
    } catch (error) {
      setLastResponse({ action: copy.load, ok: false, status: "network_error", route: READ_ONLY_007W, message: error instanceof Error ? error.message : "network_error", at: new Date().toISOString() });
    }
  };
  ...
  const submitAction = async (action: Action007W) => {
    const blockers = blockers007W(selectedApplication, checks, action, decisionReason, archiveId, copy);
    if (blockers.length) {
      setLastResponse({ action, ok: false, status: "blocked", route, message: blockers.slice(0, 4).join(" · "), at: new Date().toISOString() });
      return;
    }
    setBusyAction(action);
    const payload = buildPayload007W(action, selectedApplication, checks, decisionReason, archiveId, ownerNote, route);
    try {
      const response = await fetch(`${normalizeBase007W(config)}${route}`, { method: "POST", headers: { "content-type": "application/json", "x-sabi-admin-token": config.token || "" }, body: JSON.stringify(payload) });
      const json = await response.json().catch(() => ({}));
  ...
  return (
    <section className="messengerStyle007B taxi007wSimple taxi007wFix1" data-admin-ui-taxi-007w-fix1-applications-list-open-approve-archive="ready" data-admin-ui-taxi-007w-approved-driver-backend-payload="ready" data-admin-ui-taxi-007w-archive-all-received-data="ready">
      <div className="ms007b-hero">
        <div><span>{MARKER007W_FIX1}</span><h1>{copy.title}</h1><p>{copy.subtitle}</p></div>
        <div className="ms007b-heroStatus"><strong>{readinessPercent}%</strong><span>{copy.readiness}</span></div>
      </div>

      <div className="ms007b-tabs" data-admin-ui-taxi-007w-full-console-tabs-preserved="ready">
        {TABS007W.map((tab) => <button key={tab} type="button" className={activeTab === tab ? "active" : ""} onClick={() => setActiveTab(tab)}>{copy[tab]}</button>)}
      </div>
      ...
      {activeTab === "applications" ? (
        <div className="taxi007wApplicationsWorkspace" data-admin-ui-taxi-007w-applications-exact-workflow="ready">
          <aside className="taxi007wApplicationsList" data-admin-ui-taxi-007w-new-application-list="ready">
            <div className="ms007b-sectionTitle"><h2>{copy.newApplications}</h2><span>{filteredApplications.length}</span></div>
            <button className="taxi007wPrimary" type="button" onClick={() => void loadApplications()}>{copy.load}</button>
            <div className="taxi007wCategoryTabs" data-admin-ui-taxi-007w-application-categories="standard-comfort-business-delivery-intercity">
              {CATEGORIES007W.map((category) => <button key={category} type="button" className={activeCategory === category ? "active" : ""} onClick={() => { setActiveCategory(category); setSelectedId(""); resetReviewState(); }}>{copy.categories[category]}</button>)}
            </div>
            <div className="taxi007wListItems">
              ...
              )) : <div className="taxi007wEmptyList">{copy.noApplication}</div>}
            </div>
          </aside>

          <main className="taxi007wReviewCard" data-admin-ui-taxi-007w-click-open-application-details="ready">
            <div className="taxi007wCardHead"><div><h2>{copy.openApplication}</h2><p>{selectedApplication ? selectedApplication.applicationId : copy.noApplication}</p></div><strong>{readinessPercent}%</strong></div>
            {selectedApplication ? (
              <>
                <section className="taxi007wMiniBlock"><h3>{copy.driverData}</h3><div className="taxi007wDataGrid">{dataPairs007W(copy, selectedApplication).map(([label, value]) => <div key={label}><span>{label}</span><strong>{value || "—"}</strong></div>)}</div></section>
                <section className="taxi007wMiniBlock"><h3>{copy.carData}</h3><div className="taxi007wDataGrid">{vehiclePairs007W(copy, selectedApplication).map(([label, value]) => <div key={label}><span>{label}</span><strong>{value || "—"}</strong></div>)}</div></section>
                <section className="taxi007wMiniBlock" data-admin-ui-taxi-007w-mobile-upload-documents="ready"><h3>{copy.driverDocs}</h3><div className="taxi007wDocGrid compact">{docPairs007W(copy, selectedApplication).map(([label, value]) => <a key={label} href={value || undefined} target="_blank" rel="noreferrer" className={value ? "filled" : "missing"}><span>{label}</span><strong>{value ? copy.openApplication : copy.blocked}</strong><small>{value || copy.required}</small></a>)}</div></section>
                <section className="taxi007wMiniBlock" data-admin-ui-taxi-007w-mobile-upload-car-photos="front-back-side-interior-dashboard-mileage"><h3>{copy.carPhotos}</h3><div className="taxi007wPhotoGrid">{photoPairs007W(copy, selectedApplication).map(([label, value]) => <a key={label} href={value || undefined} target="_blank" rel="noreferrer" className={value ? "filled" : "missing"}><span>{label}</span><strong>{value ? copy.ready : copy.blocked}</strong></a>)}</div></section>
              </>
            ) : <div className="taxi007wEmptyState">{copy.noApplication}</div>}
          </main>

          <aside className="taxi007wDecisionDock" data-admin-ui-taxi-007w-approve-reject-archive-actions="ready">
            ...
            <div className="taxi007wChecks minimal" data-admin-ui-taxi-007w-minimal-checks="ready">
              {([
              ...
              ] as Array<[keyof Checks007W, string]>).map(([key, label]) => <label key={key} className={checks[key] ? "checked" : ""}><input type="checkbox" checked={checks[key]} onChange={(event) => setChecks((prev) => ({ ...prev, [key]: event.target.checked }))} /><span>{label}</span></label>)}
            </div>
            <label><span>{copy.route}</span><input value={routeOverride} onChange={(event) => setRouteOverride(event.target.value)} placeholder={WRITE_GATE_007W} /></label>
            <div className="taxi007wActionButtons exact">
            ...
            <div className="taxi007wBlockers"><strong>{copy.blocked}</strong>{approveBlockers.length ? approveBlockers.slice(0, 8).map((blocker) => <span key={blocker}>{blocker}</span>) : <span>{copy.ready}</span>}</div>
            <div className="taxi007wLastResponse"><strong>{copy.lastResponse}</strong><span>{lastResponse ? `${lastResponse.status} · ${lastResponse.message}` : copy.backendOnly}</span><small>{lastResponse?.route || route}</small></div>
            <div className="taxi007wArchivePromise"><strong>{copy.archivedIncomingData}</strong><span>{copy.approvedDriverBase}</span></div>
          </aside>
        </div>
      ) : (
        <div className="ms007b-grid" data-admin-ui-taxi-007w-other-sections-preserved="ready"><div className="ms007b-mainCard"><h2>{copy[activeTab]}</h2><p>{copy.nextSection}</p></div><div className="ms007b-sideCard"><h3>{copy.ownerAi}</h3><p>{copy.preserved}</p></div></div>
      )}
    </section>
  );
}
