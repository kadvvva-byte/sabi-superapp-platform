...
type Props007Y = { language: AdminLanguage; config: AdminApiConfig; setNotice: (message: string) => void };
type Tab007Y = "applications" | "drivers" | "vehicles" | "tariffs" | "orders" | "trips" | "complaints" | "balance" | "rewards" | "contests" | "archive" | "reports" | "access";
type Category007Y = "standard" | "comfort" | "business" | "delivery" | "intercity";
type Action007Y = "approve" | "reject" | "request-docs" | "archive";
type LastResponse007Y = { action: string; ok: boolean; status: number | string; message: string; route: string; at: string } | null;

type TaxiApplication007Y = {
  applicationId: string;
  category: Category007Y;
  status: string;
  submittedAt: string;
  driverName: string;
  phone: string;
...
type Copy007Y = {
  ...
  reports: string;
  access: string;
  categories: Record<Category007Y, string>;
...
const MARKER007Y_FIX1 = "ADMIN-UI-TAXI-007Y-APPLICATIONS-BACKEND-FOUNDATION-100";
const READ_ONLY_007Y = "/api/admin/taxi/applications/007y/new-applications";
const DETAILS_007Y = (applicationId: string) => `/api/admin/taxi/applications/007y/applications/${encodeURIComponent(applicationId)}`;
const ACTION_ROUTE_007Y = (applicationId: string, action: Action007Y) => `/api/admin/taxi/applications/007y/applications/${encodeURIComponent(applicationId)}/${action === "request-docs" ? "request-documents" : action}`;

const TABS007Y: Tab007Y[] = ["applications", "drivers", "vehicles", "tariffs", "orders", "trips", "complaints", "balance", "rewards", "contests", "archive", "reports", "access"];
const CATEGORIES007Y: Category007Y[] = ["standard", "comfort", "business", "delivery", "intercity"];

const COPY007Y: Record<AdminLanguage, Copy007Y> = {
  ru: {
...
const emptyApplication007Y: TaxiApplication007Y = {
  applicationId: "",
  category: "standard",
  status: "new",
  submittedAt: "",
...
const normalizeBase007Y = (config: AdminApiConfig): string => String(config.baseUrl || "http://127.0.0.1:3000").replace(/\/$/, "");

function pick007Y(row: Record<string, unknown>, keys: string[]): string {
...
function normalizeCategory007Y(value: string): Category007Y {
...
function fromLiveRow007Y(row: Record<string, unknown>): TaxiApplication007Y {
  return {
    applicationId: pick007Y(row, ["applicationId", "driverApplicationId", "id"]),
    category: normalizeCategory007Y(pick007Y(row, ["category", "serviceCategory", "tariffCategory", "requestedCategory"])),
    status: pick007Y(row, ["status", "applicationStatus"]) || "new",
    submittedAt: pick007Y(row, ["submittedAt", "createdAt", "receivedAt"]),
    driverName: pick007Y(row, ["driverName", "fullName", "name"]),
    phone: pick007Y(row, ["phone", "phoneNumber", "driverPhone"]),
    region: pick007Y(row, ["region", "city", "serviceRegion"]),
    driverPhotoUrl: pick007Y(row, ["driverPhotoUrl", "profilePhotoUrl", "photoUrl"]),
    passportUrl: pick007Y(row, ["passportUrl", "passportPhotoUrl", "idDocumentUrl"]),
    licenseUrl: pick007Y(row, ["licenseUrl", "licensePhotoUrl", "driverLicenseUrl"]),
    vehicleDocumentUrl: pick007Y(row, ["vehicleDocumentUrl", "registrationUrl", "registrationDocumentUrl", "vehicleRegistrationUrl"]),
    vehiclePlate: pick007Y(row, ["vehiclePlate", "plate", "plateNumber"]),
    vehicleModel: pick007Y(row, ["vehicleModel", "model", "carModel"]),
    vehicleFrontPhotoUrl: pick007Y(row, ["vehicleFrontPhotoUrl", "carFrontPhotoUrl", "frontPhotoUrl"]),
    vehicleBackPhotoUrl: pick007Y(row, ["vehicleBackPhotoUrl", "carBackPhotoUrl", "backPhotoUrl"]),
    vehicleLeftPhotoUrl: pick007Y(row, ["vehicleLeftPhotoUrl", "carLeftPhotoUrl", "leftPhotoUrl", "vehicleSidePhotoUrl"]),
    vehicleRightPhotoUrl: pick007Y(row, ["vehicleRightPhotoUrl", "carRightPhotoUrl", "rightPhotoUrl"]),
    interiorFrontPhotoUrl: pick007Y(row, ["interiorFrontPhotoUrl", "frontInteriorPhotoUrl"]),
    interiorBackPhotoUrl: pick007Y(row, ["interiorBackPhotoUrl", "backInteriorPhotoUrl", "vehicleInteriorPhotoUrl"]),
    dashboardPhotoUrl: pick007Y(row, ["dashboardPhotoUrl", "instrumentPanelPhotoUrl", "vehicleDashboardPhotoUrl"]),
    mileagePhotoUrl: pick007Y(row, ["mileagePhotoUrl", "odometerPhotoUrl", "vehicleMileagePhotoUrl"]),
    rawMobileUploadId: pick007Y(row, ["mobileUploadId", "uploadId", "rawMobileUploadId"]),
  };
}

const requiredApproveFields007Y: Array<keyof TaxiApplication007Y> = ["applicationId", "category", "driverName", "phone", "region", "driverPhotoUrl", "passportUrl", "licenseUrl", "vehicleDocumentUrl", "vehiclePlate", "vehicleModel", "vehicleFrontPhotoUrl", "vehicleBackPhotoUrl", "vehicleLeftPhotoUrl", "vehicleRightPhotoUrl", "interiorFrontPhotoUrl", "interiorBackPhotoUrl", "dashboardPhotoUrl", "mileagePhotoUrl"];

type Checks007Y = { driverDocs: boolean; vehicleDocs: boolean; vehiclePhotos: boolean; archive: boolean };

function blockers007Y(application: TaxiApplication007Y | null, checks: Checks007Y, action: Action007Y, reason: string, archiveId: string, copy: Copy007Y): string[] {
  const blockers: string[] = [];
  if (!application?.applicationId) blockers.push(`${copy.appId}: ${copy.required}`);
  if (action === "approve") {
    requiredApproveFields007Y.forEach((key) => {
...
function readiness007Y(application: TaxiApplication007Y | null, checks: Checks007Y, archiveId: string): number {
  if (!application?.applicationId) return 0;
  const filled = requiredApproveFields007Y.filter((key) => String(application[key] || "").trim()).length;
  const checked = Object.values(checks).filter(Boolean).length;
  const archive = archiveId.trim() ? 1 : 0;
  return Math.round(((filled + checked + archive) / (requiredApproveFields007Y.length + 5)) * 100);
}

function dataPairs007Y(copy: Copy007Y, application: TaxiApplication007Y): Array<[string, string]> {
...
function vehiclePairs007Y(copy: Copy007Y, application: TaxiApplication007Y): Array<[string, string]> {
  return [[copy.plate, application.vehiclePlate], [copy.model, application.vehicleModel]];
}

function docPairs007Y(copy: Copy007Y, application: TaxiApplication007Y): Array<[string, string]> {
...
function photoPairs007Y(copy: Copy007Y, application: TaxiApplication007Y): Array<[string, string]> {
...
function buildPayload007Y(action: Action007Y, application: TaxiApplication007Y | null, checks: Checks007Y, decisionReason: string, archiveId: string, ownerNote: string, route: string) {
  return {
    source: "admin-ui-007y-applications-backend-foundation-100",
    action,
...
export function TaxiAdminControl007YPanel({ language, config, setNotice }: Props007Y) {
  const copy = COPY007Y[language] || COPY007Y.ru;
  const [activeTab, setActiveTab] = useState<Tab007Y>("applications");
  const [activeCategory, setActiveCategory] = useState<Category007Y>("standard");
  const [applications, setApplications] = useState<TaxiApplication007Y[]>([]);
  const [selectedId, setSelectedId] = useState("");
  const [checks, setChecks] = useState<Checks007Y>({ driverDocs: false, vehicleDocs: false, vehiclePhotos: false, archive: false });
  const [decisionReason, setDecisionReason] = useState("");
  const [archiveId, setArchiveId] = useState("");
  const [ownerNote, setOwnerNote] = useState("");
  const [lastResponse, setLastResponse] = useState<LastResponse007Y>(null);
  ...
  const route = selectedApplication ? ACTION_ROUTE_007Y(selectedApplication.applicationId, "archive") : READ_ONLY_007Y;
  const filteredApplications = useMemo(() => applications.filter((item) => item.category === activeCategory), [applications, activeCategory]);
  const readinessPercent = readiness007Y(selectedApplication, checks, archiveId);
  const productionReadinessPercent = 83;
  const approveBlockers = useMemo(() => blockers007Y(selectedApplication, checks, "approve", decisionReason, archiveId, copy), [selectedApplication, checks, decisionReason, archiveId, copy]);
  ...
  const loadApplications = async () => {
    try {
      const response = await fetch(`${normalizeBase007Y(config)}${READ_ONLY_007Y}`, { headers: { "x-sabi-admin-token": config.token || "" } });
      ...
      const mapped = rows.map((row: Record<string, unknown>) => fromLiveRow007Y(row)).filter((item: TaxiApplication007Y) => item.applicationId && (item.status.toLowerCase().includes("new") || item.status.toLowerCase().includes("pending") || item.status.toLowerCase().includes("review")));
      setApplications(mapped);
      setSelectedId(mapped[0]?.applicationId || "");
      resetReviewState();
      setLastResponse({ action: copy.load, ok: response.ok, status: response.status, route: READ_ONLY_007Y, message: mapped.length ? `${mapped.length}` : "no_new_applications_from_backend", at: new Date().toISOString() });
      setNotice(response.ok ? copy.refresh : copy.blocked);
    } catch (error) {
      setLastResponse({ action: copy.load, ok: false, status: "network_error", route: READ_ONLY_007Y, message: error instanceof Error ? error.message : "network_error", at: new Date().toISOString() });
    }
  };
  ...
  const submitAction = async (action: Action007Y) => {
    const blockers = blockers007Y(selectedApplication, checks, action, decisionReason, archiveId, copy);
    if (blockers.length) {
      setLastResponse({ action, ok: false, status: "blocked", route, message: blockers.slice(0, 4).join(" · "), at: new Date().toISOString() });
      return;
    }
    setBusyAction(action);
    const actionRoute = selectedApplication ? ACTION_ROUTE_007Y(selectedApplication.applicationId, action) : route;
    const payload = buildPayload007Y(action, selectedApplication, checks, decisionReason, archiveId, ownerNote, actionRoute);
    try {
      const response = await fetch(`${normalizeBase007Y(config)}${actionRoute}`, { method: "POST", headers: { "content-type": "application/json", "x-sabi-admin-token": config.token || "" }, body: JSON.stringify(payload) });
      const json = await response.json().catch(() => ({}));
  ...
  return (
    <section className="messengerStyle007B taxi007ySimple taxi007yNoFake100" data-admin-ui-taxi-007y-applications-no-fake-100="ready" data-admin-ui-taxi-007y-approved-driver-backend-payload="atomic-required" data-admin-ui-taxi-007y-archive-all-received-data="required" data-admin-ui-taxi-007y-atomic-approved-driver-write="required" data-admin-ui-taxi-007y-post-write-sync="required">
      <div className="ms007b-hero">
        <div><span>{MARKER007Y_FIX1}</span><h1>{copy.title}</h1><p>{copy.subtitle}</p></div>
        <div className="ms007b-heroStatus"><strong>{readinessPercent}%</strong><span>{copy.readiness}</span></div>
      </div>

      <div className="ms007b-tabs" data-admin-ui-taxi-007y-full-console-tabs-preserved="ready">
        {TABS007Y.map((tab) => <button key={tab} type="button" className={activeTab === tab ? "active" : ""} onClick={() => setActiveTab(tab)}>{copy[tab]}</button>)}
      </div>
      ...
      {activeTab === "applications" ? (
        <div className="taxi007yApplicationsWorkspace" data-admin-ui-taxi-007y-applications-exact-workflow="ready">
          <aside className="taxi007yApplicationsList" data-admin-ui-taxi-007y-new-application-list="ready">
            <div className="ms007b-sectionTitle"><h2>{copy.newApplications}</h2><span>{filteredApplications.length}</span></div>
            <button className="taxi007yPrimary" type="button" onClick={() => void loadApplications()}>{copy.load}</button>
            <div className="taxi007yCategoryTabs" data-admin-ui-taxi-007y-application-categories="standard-comfort-business-delivery-intercity">
              {CATEGORIES007Y.map((category) => <button key={category} type="button" className={activeCategory === category ? "active" : ""} onClick={() => { setActiveCategory(category); setSelectedId(""); resetReviewState(); }}>{copy.categories[category]}</button>)}
            </div>
            <div className="taxi007yListItems">
              ...
              )) : <div className="taxi007yEmptyList">{copy.noApplication}</div>}
            </div>
          </aside>

          <main className="taxi007yReviewCard" data-admin-ui-taxi-007y-click-open-application-details="ready">
            <div className="taxi007yCardHead"><div><h2>{copy.openApplication}</h2><p>{selectedApplication ? selectedApplication.applicationId : copy.noApplication}</p></div><strong>{readinessPercent}%</strong></div>
            {selectedApplication ? (
              <>
                <section className="taxi007yMiniBlock"><h3>{copy.driverData}</h3><div className="taxi007yDataGrid">{dataPairs007Y(copy, selectedApplication).map(([label, value]) => <div key={label}><span>{label}</span><strong>{value || "—"}</strong></div>)}</div></section>
                <section className="taxi007yMiniBlock"><h3>{copy.carData}</h3><div className="taxi007yDataGrid">{vehiclePairs007Y(copy, selectedApplication).map(([label, value]) => <div key={label}><span>{label}</span><strong>{value || "—"}</strong></div>)}</div></section>
                <section className="taxi007yMiniBlock" data-admin-ui-taxi-007y-mobile-upload-documents="ready"><h3>{copy.driverDocs}</h3><div className="taxi007yDocGrid compact">{docPairs007Y(copy, selectedApplication).map(([label, value]) => <a key={label} href={value || undefined} target="_blank" rel="noreferrer" className={value ? "filled" : "missing"}><span>{label}</span><strong>{value ? copy.openApplication : copy.blocked}</strong><small>{value || copy.required}</small></a>)}</div></section>
                <section className="taxi007yMiniBlock" data-admin-ui-taxi-007y-mobile-upload-car-photos="front-back-side-interior-dashboard-mileage"><h3>{copy.carPhotos}</h3><div className="taxi007yPhotoGrid">{photoPairs007Y(copy, selectedApplication).map(([label, value]) => <a key={label} href={value || undefined} target="_blank" rel="noreferrer" className={value ? "filled" : "missing"}><span>{label}</span><strong>{value ? copy.ready : copy.blocked}</strong></a>)}</div></section>
              </>
            ) : <div className="taxi007yEmptyState">{copy.noApplication}</div>}
          </main>

          <aside className="taxi007yDecisionDock" data-admin-ui-taxi-007y-approve-reject-archive-actions="ready">
            ...
            <div className="taxi007yChecks minimal" data-admin-ui-taxi-007y-minimal-checks="ready">
              {([
              ...
              ] as Array<[keyof Checks007Y, string]>).map(([key, label]) => <label key={key} className={checks[key] ? "checked" : ""}><input type="checkbox" checked={checks[key]} onChange={(event) => setChecks((prev) => ({ ...prev, [key]: event.target.checked }))} /><span>{label}</span></label>)}
            </div>
        ...
        <div className="ms007b-grid" data-admin-ui-taxi-007y-other-sections-preserved="ready"><div className="ms007b-mainCard"><h2>{copy[activeTab]}</h2><p>{copy.nextSection}</p></div><div className="ms007b-sideCard"><h3>{copy.ownerAi}</h3><p>{copy.preserved}</p></div></div>
      )}
    </section>
  );
}
