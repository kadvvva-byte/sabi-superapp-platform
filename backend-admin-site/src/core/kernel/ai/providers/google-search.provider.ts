...
export class GoogleSearchProvider implements AiSearchProviderPort {
  ...
  constructor(private readonly gatewayUrl = process.env.AI_GOOGLE_SEARCH_GATEWAY_URL?.trim()) {}
  ...
  async search(request: AiSearchRequest): Promise<AiSearchResponse> {
    if (!this.gatewayUrl) {
      return {
        ...
        results: [],
        note: "Google search gateway is not configured. Set AI_GOOGLE_SEARCH_GATEWAY_URL to enable real search.",
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        return {
          ...
          attemptedProviders: [this.key],
          results: [],
          note: `Google search gateway returned HTTP ${response.status}`,
        }
      }
      ...
      return {
        ...
        attemptedProviders: [this.key],
        results: [],
        note: error instanceof Error ? error.message : "Unknown Google search gateway error",
      }
    }
  }
