...
type Props = {
  ...
  pickSabiLabel?: string;
  pickSabiSubtitle?: string;
  sectionLabel?: string;
  emptyLabel?: string;
  loading?: boolean;
  onClose: () => void;
  onPickDevice: () => void;
  onPickSabi?: () => void;
  onSelect: (item: ContactOption) => void;
...
export function ContactQuickSheet({
  ...
  contacts = [],
  title = "Contact",
  subtitle = "Share from device or quick list",
  pickDeviceLabel = "Pick from device",
  pickDeviceSubtitle = "contacts access / device book",
  pickSabiLabel = "Sabi contacts",
  pickSabiSubtitle = "messenger contact list",
  sectionLabel = "Quick contacts",
  emptyLabel = "No quick contacts",
  loading = false,
}: Props) {
      ...
      <View style={styles.overlay}>
        ...
        <View style={[styles.wrap, { paddingBottom: Math.max(insets.bottom, 10) }]}>
          <LinearGradient colors={["rgba(14,31,27,0.98)", "rgba(7,18,16,0.96)"]} style={[styles.card, { borderColor: `${accent}22` }]}>
            ...
            {onPickSabi ? (
              <Pressable onPress={onPickSabi} style={({ pressed }) => [styles.primaryWrap, pressed ? styles.pressed : undefined]}>
                <LinearGradient colors={["rgba(255,255,255,0.05)", "rgba(255,255,255,0.02)"]} style={[styles.primaryCard, { borderColor: `${accent}20` }]}>
                  <View style={[styles.primaryIconWrap, { borderColor: `${accent}24`, backgroundColor: `${accent}12` }]}>
                    <Ionicons name="chatbubble-ellipses-outline" size={18} color={accent} />
                  </View>
                  <View style={styles.primaryTextWrap}>
                    <Text style={styles.primaryTitle}>{pickSabiLabel}</Text>
                    <Text style={styles.primarySubtitle}>{pickSabiSubtitle}</Text>
                  </View>
                  <Ionicons name="chevron-forward" size={16} color="rgba(246,255,249,0.72)" />
                </LinearGradient>
              </Pressable>
                ...
                <Pressable key={item.id} onPress={() => onSelect(item)} style={({ pressed }) => [styles.itemWrap, pressed ? styles.pressed : undefined]}>
                  <LinearGradient colors={["rgba(255,255,255,0.045)", "rgba(255,255,255,0.02)"]} style={styles.item}>
                    ...
                    <View style={styles.itemTextWrap}>
                      ...
                      <Text style={styles.itemMeta} numberOfLines={1}>{item.phone || item.username || item.chatId}</Text>
                      {item.role ? <Text style={styles.itemRole} numberOfLines={1}>{item.role}</Text> : null}
                    </View>
                  </LinearGradient>
                </Pressable>
              )) : (
...
const styles = StyleSheet.create({
  ...
  itemRole: { marginTop: 1, color: "rgba(232,255,246,0.48)", fontSize: 10, fontWeight: "700" },
...
export default ContactQuickSheet;
