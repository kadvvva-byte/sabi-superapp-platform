...
type StreamBusinessControlSurfaceClarityPanelProps = {
  readonly version?: "124D" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const businessControls = [
  "действие продавца остаётся первым",
  "контролы каталога читаемы",
  "граница оплаты закрыта",
] as const;

export function StreamBusinessControlSurfaceClarityPanel({
  version = "124D",
  compact = false,
  style,
}: StreamBusinessControlSurfaceClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          ...
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · управление бизнесом</Text>
            <Text style={styles.title} numberOfLines={1}>Бизнес-контролы остаются понятными</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Маршрут бизнеса держит действия продавца на виду, контролы каталога — читаемыми, а границы оплаты и KYB — честно закрытыми до подключения серверной основы и маршрутов кошелька.
        </Text>

        <View style={styles.controlRow}>
          {businessControls.map((item, index) => (
