...
type StreamBusinessProofClarityPanelProps = {
  readonly version?: "128D" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const proofItems = [
  { label: "сигнал продавца", value: "видно" },
  { label: "доверие магазина", value: "ясно" },
  { label: "путь покупателя", value: "стабильно" },
] as const;

export function StreamBusinessProofClarityPanel({
  version = "128D",
  compact = false,
  style,
}: StreamBusinessProofClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          <View style={styles.iconBadge}>
            <Ionicons name="storefront-outline" size={compact ? 13 : 15} color="#061018" />
          </View>
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · доказательство бизнеса</Text>
            <Text style={styles.title} numberOfLines={1}>ясность доказательства бизнеса</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Доказательство бизнеса читается до торгового этапа: сигнал продавца, доверие магазина, путь покупателя и ограничения активации остаются ясными.
        </Text>
        ...
        <View style={styles.boundaryRow}>
          ...
          <Text style={styles.boundaryText} numberOfLines={1}>реальный торговый провайдер закрыт до фундаментного этапа</Text>
        </View>
      </LinearGradient>
    </View>
  );
