...
type StreamInteractionPathFinalCloseLockPanelProps = {
  readonly version?: "123K" | string;
...
const routeCopy: Record<RouteVariant, { icon: IconName; title: string; body: string; boundary: string }> = {
  live: {
    icon: "radio-outline",
    title: "Финальная блокировка пути взаимодействия эфира",
  ...
  shorts: {
    ...
    title: "Финальная блокировка пути взаимодействия шортов",
  ...
  business: {
    ...
    title: "Финальная блокировка пути бизнес-взаимодействия",
  ...
  profile: {
    ...
    title: "Финальная блокировка пути взаимодействия профиля",
...
const lockRows = [
  ...
  "маршрут нажатий закреплён",
  "текст закреплён",
  "граница kernel закреплена",
] as const;

export function StreamInteractionPathFinalCloseLockPanel({
  version = "123K",
  variant,
  compact = false,
  style,
}: StreamInteractionPathFinalCloseLockPanelProps) {
      ...
      >
        <View style={styles.headerRow}>
          ...
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · финальная блокировка пути взаимодействия</Text>
        ...
        <View style={styles.lockGrid}>
          {lockRows.map((row) => (
            <View key={row} style={styles.lockRowPill}>
              <Ionicons name="shield-checkmark-outline" size={12} color="#F2C75B" />
              <Text style={styles.lockRowText} numberOfLines={1}>{row}</Text>
            </View>
          ))}
        </View>
      </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  lockRowText: { color: "#FFFFFF", fontSize: 10, fontWeight: "800" },
});
