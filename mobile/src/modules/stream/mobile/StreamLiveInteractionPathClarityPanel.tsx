...
type StreamLiveInteractionPathClarityPanelProps = {
  readonly version?: "123B" | string;
...
const livePathSteps = [
  "сначала ведущий",
  "камера и микрофон спокойно",
  "реальный запуск закрыт",
] as const;

export function StreamLiveInteractionPathClarityPanel({
  version = "123B",
  compact = false,
  style,
}: StreamLiveInteractionPathClarityPanelProps) {
  return (
    <View style={[styles.wrapper, compact ? styles.wrapperCompact : null, style]}>
      <LinearGradient
        colors={["rgba(242,199,91,0.16)", "rgba(255,255,255,0.05)"]}
      ...
      >
        <View style={styles.headerRow}>
          ...
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · путь взаимодействия эфира</Text>
            <Text style={styles.title} numberOfLines={1}>Путь ведущего понятнее</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Путь эфира теперь проще: открыть настройку комнаты, подготовить устройства и дождаться реального runtime-этапа.
        </Text>
