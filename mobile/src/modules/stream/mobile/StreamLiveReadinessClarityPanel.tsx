...
type StreamLiveReadinessClarityPanelProps = {
  readonly version?: "127B" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const readinessSteps = [
  { icon: "person-circle-outline", label: "личность ведущего", value: "видна до запуска" },
  { icon: "radio-outline", label: "сигнал комнаты", value: "вход читается" },
  { icon: "people-outline", label: "доверие зрителя", value: "передача спокойная" },
] as const;

export function StreamLiveReadinessClarityPanel({
  version = "127B",
  compact = false,
  style,
}: StreamLiveReadinessClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          <View style={styles.iconBadge}>
            <Ionicons name="radio-outline" size={compact ? 14 : 16} color="#06120F" />
          </View>
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · готовность эфира</Text>
            <Text style={styles.title} numberOfLines={1}>ясность готовности эфира</Text>
          </View>
          <View style={styles.statusBadge}>
            <Ionicons name="lock-closed-outline" size={12} color="#06120F" />
            <Text style={styles.statusText}>запуск закрыт</Text>
          </View>
        </View>

        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Готовность комнаты эфира остаётся ясной до реальной привязки провайдера: личность ведущего, сигнал комнаты, доверие зрителя и граница запуска читаются без притворства, что комната уже в эфире.
        </Text>
        ...
        <View style={styles.boundaryRow}>
          ...
          <Text style={styles.boundaryText} numberOfLines={1}>только мобильный интерфейс · реальный провайдер эфира остаётся закрытым</Text>
        </View>
      </LinearGradient>
    </View>
  );
