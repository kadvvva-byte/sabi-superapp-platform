...
type StreamLiveRoomFlowClarityPanelProps = {
  readonly version?: "126B" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const flowRows = [
  { label: "путь входа", value: "ясно", icon: "enter-outline" },
  { label: "действие ведущего", value: "видно", icon: "radio-outline" },
  { label: "переход к зрителю", value: "стабильно", icon: "people-outline" },
] as const;

export function StreamLiveRoomFlowClarityPanel({
  version = "126B",
  compact = false,
  style,
}: StreamLiveRoomFlowClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          <View style={styles.iconBadge}>
            <Ionicons name="git-branch-outline" size={compact ? 14 : 16} color="#0B0810" />
          </View>
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · Путь live-комнаты</Text>
            <Text style={styles.title} numberOfLines={1}>Путь live-комнаты остаётся понятным</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Live-маршрут понятно показывает вход в комнату, действие ведущего, переход к зрителю и границу реального провайдера до подключения streaming-runtime через kernel-слой.
        </Text>
        ...
        <View style={styles.boundaryRow}>
          ...
          <Text style={styles.boundaryText} numberOfLines={1}>Только мобильный UI · активация live-провайдера позже</Text>
        </View>
      </LinearGradient>
    </View>
  );
