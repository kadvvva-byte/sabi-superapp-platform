...
type StreamProfileControlSurfaceClarityPanelProps = {
  readonly version?: "124E" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const profileControls = [
  "действие автора остаётся первым",
  "контролы профиля читаемы",
  "граница заработка закрыта",
] as const;

export function StreamProfileControlSurfaceClarityPanel({
  version = "124E",
  compact = false,
  style,
}: StreamProfileControlSurfaceClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          ...
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · управление профилем</Text>
            <Text style={styles.title} numberOfLines={1}>Контролы автора остаются понятными</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Маршрут профиля держит действия автора на виду, контролы профиля — читаемыми, а границы заработка и верификации — честно закрытыми до подключения реального аккаунта и review flows.
        </Text>

        <View style={styles.controlRow}>
          {profileControls.map((item, index) => (
