...
type StreamProfileInteractionPathClarityPanelProps = {
  readonly version?: "123E" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const profilePathSteps = [
  "личность автора понятна",
  "верификация честная",
  "заработок закрыт",
] as const;

export function StreamProfileInteractionPathClarityPanel({
  version = "123E",
  compact = false,
  style,
}: StreamProfileInteractionPathClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          ...
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · путь взаимодействия профиля</Text>
            <Text style={styles.title} numberOfLines={1}>Путь автора понятнее</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Путь профиля сначала показывает личность автора, оставляет официальную верификацию честной, а подарки, заработок, Wallet и выплаты закрытыми до реального runtime.
        </Text>

        <View style={styles.stepRow}>
          {profilePathSteps.map((step, index) => (
