...
type StreamProfileProofClarityPanelProps = {
  readonly version?: "128E" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const proofItems = [
  { label: "сигнал автора", value: "видно" },
  { label: "доверие профиля", value: "ясно" },
  { label: "путь зрителя", value: "стабильно" },
] as const;

export function StreamProfileProofClarityPanel({
  version = "128E",
  compact = false,
  style,
}: StreamProfileProofClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          <View style={styles.iconBadge}>
            <Ionicons name="person-circle-outline" size={compact ? 13 : 15} color="#061018" />
          </View>
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · доказательство профиля</Text>
            <Text style={styles.title} numberOfLines={1}>ясность доказательства профиля</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Доказательство профиля читается до этапа провайдера: сигнал автора, доверие профиля, путь зрителя и ограничения проверки остаются ясными.
        </Text>
        ...
        <View style={styles.boundaryRow}>
          ...
          <Text style={styles.boundaryText} numberOfLines={1}>реальный провайдер проверки закрыт до фундаментного этапа</Text>
        </View>
      </LinearGradient>
    </View>
  );
