...
type StreamProfileRouteSpecificMobilePolishPanelProps = {
  readonly version?: "121E" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const profileRows: readonly { icon: IconName; title: string; body: string }[] = [
  {
    icon: "person-circle-outline",
    title: "Сначала личность автора",
    body: "Маршрут профиля начинается с личности автора, прогресса настройки и честного статуса проверки, а не с внутренних инструментов.",
  },
  {
    icon: "shield-checkmark-outline",
    title: "Проверка остаётся реальной",
    body: "Официальный значок, одобрение автора и готовность заработка остаются видимыми воротами до реальной проверки.",
  },
  {
    icon: "grid-outline",
    title: "Действия профиля понятны",
    body: "Настройка, регистрация и карточки профиля сохраняют удобный мобильный порядок без debug-текста.",
  },
  {
    icon: "lock-closed-outline",
    title: "Блокировка заработка остаётся честной",
    body: "Подарки, монетизация, кошелёк, выплаты и премиальный заработок автора отложены до следующих фаз Stream.",
  },
];

const guardRows = [
  "Только мобильный интерфейс профиля автора",
  "Без фейковых подписчиков, значка или успешной проверки",
  "Без активации подарков, заработка, кошелька или выплат",
  "Без изменений backend, провайдера, админ-панели, Messenger или ИИ",
] as const;

export function StreamProfileRouteSpecificMobilePolishPanel({
  version = "121E",
  compact = false,
  style,
}: StreamProfileRouteSpecificMobilePolishPanelProps) {
  const [open, setOpen] = useState(false);
  const readyCount = useMemo(() => profileRows.length, []);
  ...
  return (
    <View style={[styles.wrapper, compact ? styles.wrapperCompact : null, style]}>
      <Pressable
        ...
        accessibilityLabel={`${version} Полировка мобильного маршрута профиля`}
      >
        <LinearGradient
        ...
        >
          <View style={styles.leftSide}>
            <View style={styles.iconBadge}>
              <Ionicons name="person-circle-outline" size={compact ? 14 : 16} color="#09070D" />
            </View>
            <View style={styles.copyWrap}>
              <Text style={styles.title} numberOfLines={1}>121E · Полировка маршрута профиля</Text>
              <Text style={styles.meta} numberOfLines={1}>Личность автора, реальная проверка и честная блокировка заработка.</Text>
            </View>
          </View>
          ...
          <Pressable style={styles.sheet}>
            <View style={styles.sheetTop}>
              <View style={styles.sheetIcon}>
                <Ionicons name="person-circle-outline" size={24} color="#F2C75B" />
              </View>
              <View style={styles.sheetTitleWrap}>
                <Text style={styles.sheetTitle}>{version} · Полировка мобильного маршрута профиля</Text>
                <Text style={styles.sheetMeta}>Маршрутный проход полировки профиля автора до реального заработка и runtime проверки.</Text>
              </View>
            ...
            <View style={styles.noticeBox}>
              ...
              <Text style={styles.noticeText} numberOfLines={2}>
                Профиль автора остаётся чистой мобильной поверхностью настройки. Официальный статус, заработок и выплаты остаются заблокированы до реальной проверки и финансовых слоёв.
              </Text>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
              ...
              <View style={styles.guardBox}>
                <Text style={styles.guardTitle}>121E · границы этапа</Text>
                {guardRows.map((rule) => (
              ...
              <View style={styles.nextBox}>
                ...
                <Text style={styles.nextBody}>Продолжить маршрутную приёмку эфира, шортов, бизнеса и профиля перед переходом к следующей мобильной зоне интерфейса.</Text>
              </View>
            </ScrollView>
          </Pressable>
        </Pressable>
