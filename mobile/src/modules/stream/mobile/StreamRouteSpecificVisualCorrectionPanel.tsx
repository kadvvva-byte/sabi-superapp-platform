import React, { useMemo, useState } from "react";
import { Modal, Pressable, ScrollView, StyleProp, StyleSheet, Text, View, ViewStyle } from "react-native";
import { Ionicons } from "@expo/vector-icons";
import { LinearGradient } from "expo-linear-gradient";

type StreamRouteSpecificVisualCorrectionVariant = "shell" | "live" | "shorts" | "business" | "profile";
type StreamRouteSpecificVisualCorrectionRoute = Exclude<StreamRouteSpecificVisualCorrectionVariant, "shell">;
type IconName = keyof typeof Ionicons.glyphMap;

type StreamRouteSpecificVisualCorrectionPanelProps = {
  readonly version?: "120J" | string;
  readonly variant?: StreamRouteSpecificVisualCorrectionVariant;
  readonly ordinaryUserMode?: boolean;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
  readonly onOpenRoute?: (route: StreamRouteSpecificVisualCorrectionRoute) => void;
};

const variantCopy: Record<StreamRouteSpecificVisualCorrectionVariant, { title: string; meta: string; icon: IconName; focus: string }> = {
  shell: {
    title: "120J · Визуальная правка маршрутов",
    meta: "Финальная визуальная проверка маршрутов мобильной оболочки Stream.",
    icon: "grid-outline",
    focus: "Главный вход Stream должен выглядеть как единый чистый продукт до любого этапа реального движка.",
  },
  live: {
    title: "120J · Визуальная правка эфира",
    meta: "Настройка эфира держит путь ведущего спокойным, читаемым и честным.",
    icon: "radio-outline",
    focus: "Эфир должен показывать настройку и закрытые состояния провайдера без имитации трансляции.",
  },
  shorts: {
    title: "120J · Визуальная правка шортов",
    meta: "Шорты чётко разделяют просмотр, создание и редактирование.",
    icon: "play-circle-outline",
    focus: "Шорты не должны выглядеть опубликованными или загруженными до реального провайдерного этапа.",
  },
  business: {
    title: "120J · Визуальная правка бизнеса",
    meta: "Бизнес-стрим остаётся премиальным, пока мерчант и оплата закрыты.",
    icon: "briefcase-outline",
    focus: "Бизнес-экраны не должны показывать фейковые товары, фейковые заказы или фейковый успех оплаты.",
  },
  profile: {
    title: "120J · Визуальная правка профиля",
    meta: "Профиль автора честно показывает публичную настройку и официальный статус.",
    icon: "person-circle-outline",
    focus: "Профиль не должен показывать фейковых подписчиков, доход, бейджи или фейковый успех верификации.",
  },
};

const routeOrder: readonly { route: StreamRouteSpecificVisualCorrectionRoute; label: string; icon: IconName }[] = [
  { route: "live", label: "Эфир", icon: "radio-outline" },
  { route: "shorts", label: "Шорты", icon: "play-circle-outline" },
  { route: "business", label: "Бизнес", icon: "briefcase-outline" },
  { route: "profile", label: "Профиль", icon: "person-circle-outline" },
];

const correctionRows: readonly { icon: IconName; title: string; body: string }[] = [
  {
    icon: "layers-outline",
    title: "Верхний блок остаётся продуктовым",
    body: "Первая видимая карточка объясняет маршрут до появления глубоких закрытых состояний реального движка.",
  },
  {
    icon: "resize-outline",
    title: "Визуальный вес остаётся сбалансированным",
    body: "Карточки используют спокойные отступы, чтобы закрытые состояния выглядели намеренно, а не сломанно.",
  },
  {
    icon: "hand-left-outline",
    title: "Путь касания остаётся удобным",
    body: "Чипы маршрутов и действия в панели сохраняют понятные зоны касания для телефона.",
  },
  {
    icon: "lock-closed-outline",
    title: "Закрытые состояния остаются честными",
    body: "Провайдер, публикация, оплата, Wallet, подарки, доход и Admin остаются закрытыми до реальных этапов.",
  },
];

const scopeRows = [
  "Только правка мобильного UI маршрутов Stream",
  "Без запуска сервера, провайдера, Admin или реального движка",
  "Без изменений Wallet, Messenger, AI, подарков, платежей или монетизации",
  "Без фейкового эфира, загрузки, заказа, подписчиков, баланса или состояния одобрения",
] as const;

export function StreamRouteSpecificVisualCorrectionPanel({
  version = "120J",
  variant = "shell",
  ordinaryUserMode = true,
  compact = false,
  style,
  onOpenRoute,
}: StreamRouteSpecificVisualCorrectionPanelProps) {
  const [open, setOpen] = useState(false);
  const copy = variantCopy[variant];
  const readyCount = useMemo(() => correctionRows.length, []);

  return (
    <View style={[styles.wrapper, compact ? styles.wrapperCompact : null, style]}>
      <Pressable style={styles.card} onPress={() => setOpen(true)} accessibilityRole="button" accessibilityLabel={`${version} визуальная правка маршрутов Stream`}>
        <LinearGradient colors={["rgba(242,199,91,0.09)", "rgba(255,255,255,0.032)"]} style={[styles.cardGradient, compact ? styles.cardGradientCompact : null]}>
          <View style={styles.leftSide}>
            <View style={styles.iconBadge}>
              <Ionicons name={copy.icon} size={compact ? 14 : 16} color="#09070D" />
            </View>
            <View style={styles.copyWrap}>
              <Text style={styles.title} numberOfLines={1}>{copy.title}</Text>
              <Text style={styles.meta} numberOfLines={1}>{copy.meta}</Text>
            </View>
          </View>
          <View style={styles.readyPill}>
            <Text style={styles.readyText}>{readyCount}/4</Text>
            <Ionicons name="chevron-forward" size={14} color="#09070D" />
          </View>
        </LinearGradient>
      </Pressable>

      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        <Pressable style={styles.backdrop} onPress={() => setOpen(false)}>
          <Pressable style={styles.sheet}>
            <View style={styles.sheetTop}>
              <View style={styles.sheetIcon}>
                <Ionicons name="color-filter-outline" size={24} color="#F2C75B" />
              </View>
              <View style={styles.sheetTitleWrap}>
                <Text style={styles.sheetTitle}>{version} · Stream route visual correction</Text>
                <Text style={styles.sheetMeta}>Final tiny correction layer for route-specific mobile hierarchy, spacing and honest locked states.</Text>
              </View>
              <Pressable style={styles.closeButton} onPress={() => setOpen(false)} accessibilityRole="button" accessibilityLabel="Закрыть">
                <Ionicons name="close" size={21} color="#FFFFFF" />
              </Pressable>
            </View>

            <View style={styles.modeNotice}>
              <View style={ordinaryUserMode ? styles.modeDot : styles.modeDotOff} />
              <Text style={styles.modeNoticeText} numberOfLines={2}>
                {ordinaryUserMode ? copy.focus : "Обычный пользовательский режим должен оставаться включённым до принятия публичной правки маршрутов."}
              </Text>
            </View>

            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
              <View style={styles.routeStrip}>
                {routeOrder.map((item) => (
                  <Pressable
                    key={item.route}
                    style={[styles.routeChip, item.route === variant ? styles.routeChipActive : null]}
                    onPress={() => onOpenRoute?.(item.route)}
                    accessibilityRole="button"
                    accessibilityLabel={`Открыть ${item.label}`}
                  >
                    <Ionicons name={item.icon} size={16} color={item.route === variant ? "#09070D" : "#F2C75B"} />
                    <Text style={[styles.routeChipText, item.route === variant ? styles.routeChipTextActive : null]}>{item.label}</Text>
                  </Pressable>
                ))}
              </View>

              <View style={styles.correctionGrid}>
                {correctionRows.map((item) => (
                  <View key={item.title} style={styles.correctionCard}>
                    <View style={styles.correctionIcon}>
                      <Ionicons name={item.icon} size={20} color="#F2C75B" />
                    </View>
                    <View style={styles.correctionTextWrap}>
                      <Text style={styles.correctionTitle}>{item.title}</Text>
                      <Text style={styles.correctionBody}>{item.body}</Text>
                    </View>
                  </View>
                ))}
              </View>

              <View style={styles.scopeBox}>
                <Text style={styles.scopeTitle}>Correction scope</Text>
                {scopeRows.map((rule) => (
                  <View key={rule} style={styles.scopeRow}>
                    <Ionicons name="checkmark-circle-outline" size={15} color="#F2C75B" />
                    <Text style={styles.scopeText}>{rule}</Text>
                  </View>
                ))}
              </View>

              <View style={styles.nextBox}>
                <Text style={styles.nextTitle}>Next UI focus</Text>
                <Text style={styles.nextBody}>Continue only with small visual fixes if a real route still feels uneven on the phone. Runtime, backend, gifts, Wallet and Admin stay for later.</Text>
              </View>
            </ScrollView>
          </Pressable>
        </Pressable>
      </Modal>
    </View>
  );
}

const styles = StyleSheet.create({
  wrapper: { marginBottom: 10 },
  wrapperCompact: { marginBottom: 7 },
  card: { borderRadius: 22, overflow: "hidden" },
  cardGradient: { minHeight: 56, borderRadius: 22, borderWidth: 1, borderColor: "rgba(242,199,91,0.14)", paddingHorizontal: 10, paddingVertical: 8, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 8 },
  cardGradientCompact: { minHeight: 48, paddingVertical: 6 },
  leftSide: { flex: 1, minWidth: 0, flexDirection: "row", alignItems: "center", gap: 9 },
  iconBadge: { width: 34, height: 34, borderRadius: 15, backgroundColor: "#F2C75B", alignItems: "center", justifyContent: "center" },
  copyWrap: { flex: 1, minWidth: 0 },
  title: { color: "#FFFFFF", fontSize: 13, fontWeight: "900" },
  meta: { color: "#D8D1E1", fontSize: 10, fontWeight: "800", marginTop: 3 },
  readyPill: { minWidth: 46, height: 29, borderRadius: 15, backgroundColor: "#F2C75B", paddingHorizontal: 7, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 2 },
  readyText: { color: "#09070D", fontSize: 11, fontWeight: "900" },
  backdrop: { flex: 1, backgroundColor: "rgba(0,0,0,0.72)", justifyContent: "flex-end" },
  sheet: { maxHeight: "88%", borderTopLeftRadius: 30, borderTopRightRadius: 30, backgroundColor: "#111018", borderWidth: 1, borderColor: "#292532", paddingHorizontal: 16, paddingTop: 16, paddingBottom: 24 },
  sheetTop: { flexDirection: "row", alignItems: "center", gap: 12 },
  sheetIcon: { width: 48, height: 48, borderRadius: 20, backgroundColor: "rgba(242,199,91,0.10)", borderWidth: 1, borderColor: "rgba(242,199,91,0.20)", alignItems: "center", justifyContent: "center" },
  sheetTitleWrap: { flex: 1, minWidth: 0 },
  sheetTitle: { color: "#FFFFFF", fontSize: 18, fontWeight: "900" },
  sheetMeta: { color: "#8D8796", fontSize: 11, fontWeight: "800", marginTop: 4, lineHeight: 15 },
  closeButton: { width: 40, height: 40, borderRadius: 20, backgroundColor: "rgba(255,255,255,0.08)", borderWidth: 1, borderColor: "rgba(255,255,255,0.12)", alignItems: "center", justifyContent: "center" },
  modeNotice: { minHeight: 48, borderRadius: 20, backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", paddingHorizontal: 12, flexDirection: "row", alignItems: "center", gap: 8, marginTop: 12 },
  modeDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: "#F2C75B" },
  modeDotOff: { width: 7, height: 7, borderRadius: 4, backgroundColor: "#FF6B6B" },
  modeNoticeText: { flex: 1, color: "#E1DCE8", fontSize: 11, fontWeight: "900", lineHeight: 15 },
  content: { paddingTop: 12, gap: 12 },
  routeStrip: { flexDirection: "row", flexWrap: "wrap", gap: 8 },
  routeChip: { minHeight: 34, borderRadius: 17, borderWidth: 1, borderColor: "rgba(242,199,91,0.22)", backgroundColor: "rgba(242,199,91,0.08)", paddingHorizontal: 11, flexDirection: "row", alignItems: "center", gap: 6 },
  routeChipActive: { backgroundColor: "#F2C75B", borderColor: "#F2C75B" },
  routeChipText: { color: "#F2C75B", fontSize: 11, fontWeight: "900" },
  routeChipTextActive: { color: "#09070D" },
  correctionGrid: { gap: 9 },
  correctionCard: { minHeight: 74, borderRadius: 22, backgroundColor: "rgba(255,255,255,0.055)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", padding: 12, flexDirection: "row", gap: 11 },
  correctionIcon: { width: 36, height: 36, borderRadius: 16, backgroundColor: "rgba(242,199,91,0.10)", borderWidth: 1, borderColor: "rgba(242,199,91,0.18)", alignItems: "center", justifyContent: "center" },
  correctionTextWrap: { flex: 1, minWidth: 0 },
  correctionTitle: { color: "#FFFFFF", fontSize: 13, fontWeight: "900" },
  correctionBody: { color: "#AFA8BB", fontSize: 11, fontWeight: "800", lineHeight: 16, marginTop: 3 },
  scopeBox: { borderRadius: 22, backgroundColor: "rgba(242,199,91,0.08)", borderWidth: 1, borderColor: "rgba(242,199,91,0.18)", padding: 12, gap: 8 },
  scopeTitle: { color: "#FFFFFF", fontSize: 13, fontWeight: "900" },
  scopeRow: { flexDirection: "row", alignItems: "center", gap: 7 },
  scopeText: { flex: 1, color: "#DCD6E4", fontSize: 11, fontWeight: "800", lineHeight: 15 },
  nextBox: { borderRadius: 20, backgroundColor: "rgba(255,255,255,0.05)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", padding: 12 },
  nextTitle: { color: "#FFFFFF", fontSize: 12, fontWeight: "900" },
  nextBody: { color: "#AFA8BB", fontSize: 11, fontWeight: "800", lineHeight: 16, marginTop: 4 },
});
