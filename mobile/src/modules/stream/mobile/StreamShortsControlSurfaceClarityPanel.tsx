...
type StreamShortsControlSurfaceClarityPanelProps = {
  readonly version?: "124C" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const shortsControls = [
  "действие зрителя остаётся первым",
  "контролы редактора читаемы",
  "граница публикации закрыта",
] as const;

export function StreamShortsControlSurfaceClarityPanel({
  version = "124C",
  compact = false,
  style,
}: StreamShortsControlSurfaceClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          <View style={styles.iconBadge}>
            <Ionicons name="play-circle-outline" size={compact ? 14 : 16} color="#0B0810" />
          </View>
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · управление шортами</Text>
            <Text style={styles.title} numberOfLines={1}>Контролы шортов остаются понятными</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Маршрут шортов держит действия зрителя на виду, инструменты автора — читаемыми, а границы публикации и загрузки — честно закрытыми до подключения backend/provider runtime.
        </Text>

        <View style={styles.controlRow}>
          {shortsControls.map((item, index) => (
