...
type StreamShortsInteractionPathClarityPanelProps = {
  readonly version?: "123C" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const shortsPathSteps = [
  "сначала контент",
  "редактор понятен",
  "публикация закрыта",
] as const;

export function StreamShortsInteractionPathClarityPanel({
  version = "123C",
  compact = false,
  style,
}: StreamShortsInteractionPathClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          ...
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · путь взаимодействия шортов</Text>
            <Text style={styles.title} numberOfLines={1}>Путь шортов понятнее</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Путь шортов сначала показывает просмотр, даёт спокойный редактор и оставляет реальную публикацию/загрузку для runtime-этапа.
        </Text>

        <View style={styles.stepRow}>
          {shortsPathSteps.map((step, index) => (
