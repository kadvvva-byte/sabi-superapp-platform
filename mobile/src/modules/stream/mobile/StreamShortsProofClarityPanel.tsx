...
type StreamShortsProofClarityPanelProps = {
  readonly version?: "128C" | string;
...
const proofItems = [
  ...
  { label: "доверие черновика", value: "ясно" },
  { label: "путь зрителя", value: "стабильно" },
] as const;

export function StreamShortsProofClarityPanel({
  version = "128C",
  compact = false,
  style,
}: StreamShortsProofClarityPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          <View style={styles.iconBadge}>
            <Ionicons name="film-outline" size={compact ? 13 : 15} color="#061018" />
          </View>
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · доказательство шортов</Text>
            <Text style={styles.title} numberOfLines={1}>ясность доказательства шортов</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Доказательство шортов читается до этапа загрузки: сигнал автора, доверие черновика, путь зрителя и ограничения публикации остаются ясными.
        </Text>
        ...
        <View style={styles.boundaryRow}>
          ...
          <Text style={styles.boundaryText} numberOfLines={1}>реальный провайдер загрузки закрыт до фундаментного этапа</Text>
        </View>
      </LinearGradient>
    </View>
  );
