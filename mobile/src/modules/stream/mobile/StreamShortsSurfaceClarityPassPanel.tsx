...
type StreamShortsSurfaceClarityPassPanelProps = {
  readonly version?: "122C" | string;
  readonly compact?: boolean;
  readonly style?: StyleProp<ViewStyle>;
};

const shortsSteps = [
  "поверхность зрителя чистая",
  "путь редактора спокойный",
  "граница публикации закрыта",
] as const;

export function StreamShortsSurfaceClarityPassPanel({
  version = "122C",
  compact = false,
  style,
}: StreamShortsSurfaceClarityPassPanelProps) {
  return (
      ...
      >
        <View style={styles.headerRow}>
          ...
          <View style={styles.titleWrap}>
            <Text style={styles.kicker} numberOfLines={1}>{version} · ясность Shorts-поверхности</Text>
            <Text style={styles.title} numberOfLines={1}>Просмотр Shorts стал собранным и чистым</Text>
          </View>
        ...
        <Text style={styles.body} numberOfLines={compact ? 2 : 3}>
          Зритель видит простую mobile-first поверхность Shorts, а монтаж, загрузка, provider-публикация и заработок остаются закрытыми до реального runtime-этапа.
        </Text>

        <View style={styles.stepRow}>
          {shortsSteps.map((item) => (
