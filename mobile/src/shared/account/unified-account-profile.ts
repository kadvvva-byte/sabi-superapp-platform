...
export async function saveUnifiedAccountProfile(input: SaveUnifiedAccountProfileInput): Promise<UnifiedAccountProfile> {
  ...
  await withLocalProfileTimeout(
    "updateProfile",
    profileKernelFacade.updateProfile(toKernelAccount(unified, effectiveApiProfile, { ...current, ...submittedFallback }) as any),
  );

  await withLocalProfileTimeout(
    "updatePublicProfile",
    profileKernelFacade.updatePublicProfile({
      publicName: effectiveApiProfile.displayName,
      publicUsername: effectiveApiProfile.username,
      publicBio: effectiveApiProfile.bio || "",
